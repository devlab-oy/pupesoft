...
function generoi_tyomaaraykset_huoltosykleista($laitteiden_huoltosyklirivit) {
	...
	if ($debug) {
		echo "TY÷MƒƒRƒYKSIƒ PITƒISI TULLA ".count($laitteiden_huoltosyklirivit).' kappaletta';
		echo "<br/>";
	}
	foreach ($laitteiden_huoltosyklirivit as $huoltosyklirivi) {
		if ($huoltosyklirivi['toimenpide_tuotteen_tyyppi'] == 'koeponnistus' and !empty($huoltosyklirivi['viimeinen_tapahtuma'])) {
			...
			//jos seuraavaan koeponnistukseen on enemm‰n kuin 2kk aikaa, ei generoida tyˆm‰‰r‰yst‰.
		...
		if ($onko_tyomaarays_jo_luotu_talle_laitteelle) {
			if ($debug) {
				echo "T‰lle laitteelle ".$huoltosyklirivi['laite_tunnus']." on jo luotu tyˆm‰‰r‰ys";
				echo "<br/>";
			}
			continue;
		}
		...
		if (!$onko_toimenpide_tuote_saldoton) {
			if ($debug) {
				echo "Toimenpide tuote ".$huoltosyklirivi['laite_tunnus']." pit‰‰ olla saldoton! Tyˆm‰‰r‰yst‰ t‰lle tuotteelle ei lis‰tty";
				echo "<br/>";
			}
			continue;
		}

...
if (php_sapi_name() == 'cli') {
  ...
  if (!empty($tunnukset)) {
    echo "VIRHE: Tilauksen tunnus / tunnukset puuttuvat!<br>";
    require "inc/footer.inc";
    exit;
  }

  # resend_logmaster.php:sta t‰ytyy saada tilauksen tunnukset
  $tunnuksetlisa = "AND tunnus IN ({$tunnukset})";
}
...
while ($laskurow = mysql_fetch_assoc($laskures)) {
  ...
  if (file_put_contents($filename, $xml->asXML())) {
    ...
    if ($palautus == 0) {
      ...
      if (!$_cli) {
        echo "<br /><font class='message'>", t("Siirretiin tilaus %d %s -j‰rjestelm‰‰n.", "", $otunnus, $uj_nimi), "</font><br />";
      }
    }
    else {
      ...
      if (!$_cli) {
        echo "<br /><font class='message'>", t("Tilauksen %d siirto %s -j‰rjestelm‰‰n ep‰onnistui.", "", $otunnus, $uj_nimi), "</font><br />";
      }
    }
  }
  else {
    ...
    if (!$_cli) {
      echo "<br /><font class='message'>", t("Tilauksen %d sanoman luonti %s -j‰rjestelm‰‰n ep‰onnistui.", "", $otunnus, $uj_nimi), "</font><br />";
    }
  }
}

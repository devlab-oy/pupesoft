...
echo "<font class='head'>" . t ( "Kuvien sis‰‰nluku lajikoodin mukaan" ) . "</font><hr>";
...
if ($tee == 'GO') {
	...
	foreach ( $files as $file ) {
		...
		if (strpos ( $kuva, "%" ) !== FALSE) {
			...
			//kyseess‰ on k‰ytt‰turvatiedot ja tuotekortti
			if (strpos ( $kuva, "%ktt" ) !== FALSE) {
				$mistakieli = strpos ( $kuva, "%ktt" ) + 4;
				$mikakieli = substr ( $kuva, $mistakieli, 2 );
				
				if (strpos ( $mikakieli, "fi" ) !== FALSE or strpos ( $mikakieli, "se" ) !== FALSE or strpos ( $mikakieli, "en" ) !== FALSE or strpos ( $mikakieli, "ru" ) !== FALSE or strpos ( $mikakieli, "ee" ) !== FALSE or strpos ( $mikakieli, "no" ) !== FALSE or strpos ( $mikakieli, "de" ) !== FALSE) {
				...
				if (strpos ( $mikakieli, "fi" ) !== FALSE or strpos ( $mikakieli, "se" ) !== FALSE or strpos ( $mikakieli, "en" ) !== FALSE or strpos ( $mikakieli, "ru" ) !== FALSE or strpos ( $mikakieli, "no" ) !== FALSE or strpos ( $mikakieli, "ee" ) !== FALSE or strpos ( $mikakieli, "de" ) !== FALSE) {
			...
			$query = "SELECT lajikoodi, tunnus FROM tuote WHERE yhtio = '$kukarow[yhtio]' AND lajikoodi LIKE '$kuvanalku%'";
		...
		if (file_exists ( $file )) {
			...
			if (! isset ( $apuresult )) {
				...
				$query = "SELECT lajikoodi, tunnus FROM tuote WHERE yhtio = '$kukarow[yhtio]' AND lajikoodi = '$tuoteno' LIMIT 1";
				$apuresult = mysql_query ( $query ) or pupe_error ( $query );
			}
			...
			if (mysql_num_rows ( $apuresult ) > 0) {
				...
				echo " &raquo; <font class='error'>Ohitetaan kuva, koska kuvalle ei l‰ytynyt tuotetta!</font><br>";
			}
		}
	}
	echo "<br>";
...
if ($tee == 'DUMPPAA' and $mitkadumpataan != '') {
	...
	while ( $row = mysql_fetch_array ( $result ) ) {
		...
		if ($row ["kayttotarkoitus"] == "TH") {
			...
			echo "<font class='message'>Tuntematon k‰ytt‰tarkoitus $row[kayttotarkoitus]!</font>";
			continue;
		}

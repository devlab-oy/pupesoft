...
if ($tee == 'N') {
  ...
  if ($asaldo == 0) {
    echo "<font class='error'>".t("Anna siirrettävä määrä")."!</font><br><br>";
...
if ($tee == 'N') {
  ...
  if (mysql_num_rows($result) == 0) {
    echo "<font class='error'>".t("Tämä varastopaikka katosi tuotteelta")." $otetaan[0]</font><br><br>";
    $tee = $uusitee;
  }

  // Minne varastoon viedään?
  $query = "SELECT *
            FROM tuotepaikat
  ...
  if (mysql_num_rows($result) == 0) {
    echo "<font class='error'>".t("Tämä varastopaikka katosi tuotteelta")." $siirretaan[0]</font><br><br>";
    $tee = $uusitee;
  }

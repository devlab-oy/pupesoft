			...
			foreach($tuote as $i => $tuotteet) {
				...
				if ($kpl != '' and is_numeric($kpl)) {
					...
					if (isset($paivamaaran_kasisyotto) and !empty($inventointipvm)) {
						...
						if (!$koppi) {
							// Katsotaan onko inventointipäivä syötetty käsin ja fiilataan se kuntoon
							...
							if (!empty($laadittuaika) and $laadittuaika != "now()") {
								...
								if (mysql_num_rows($ressu) > 0) {
									echo "<font class='error'>".t("VIRHE: Virheellinen inventointipäivämäärä")."!: $tuoteno ".t("Tuotteella on inventointiin vaikuttavia tapahtumia annetun päivämäärän jälkeen")."</font><br>";
									$virhe = 1;
								}
								else {

...
if ($lopetus == "") {
  ...
  "</option>
    ...
    "<br>(", t("Kaikkia annettuja tuotteita tulee lˆyty‰"), ")</th><td colspan='3'><textarea name='tuotteet_lista2' rows='5' cols='35'>{$tuotteet_lista2}</textarea></td>
  ...
  echo "<tr>";
  echo "<th>".t('N‰yt‰ tilauksen luokittelu')."</th>";
...
if ((isset($aja_raportti) or isset($valitse_asiakas)) and count($_REQUEST) > 0) {
  ...
  if ($tee == 'go') {
    ...
    if (isset($tuotteet_lista2) and $tuotteet_lista2 != '') {
      ...
      // ei voida groupata tuotteittain t‰ss‰ keississ‰
      if ($tuotegroups > 0) {
        echo "<font class='error'>".t("VIRHE: Tulosta ei voi groupata tuotteilla kun k‰ytet‰‰n tuotelista 2 rajausta")."!</font><br>";
        #$tee = '';
      }
      ...
      foreach ($tuotteet as $tuote) {
        if (trim($tuote) != '') {
          ...
          if ($ajotapa == 'tilausjaauki') {
            ...
            sum(if(tilausrivi.tuoteno = '{$tuote}' and {$pvm_vertailu2}, {$kpl_kentta2}, 0)) Tuote{$_i}_{$tuote}";
          }
          else {
            $tuotelista2_select .= ", sum(if(tilausrivi.tuoteno = '{$tuote}' and {$pvm_vertailu}, {$kpl_kentta}, 0)) Tuote{$_i}_{$tuote}";
          }

          $tuotelista2_having .= "Tuote{$_i}_{$tuote} > 0 and ";
          $_i++;
        }
      }

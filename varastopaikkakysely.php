...
# Voidaan hakea sscc-koodilla jos suuntalavat on k‰ytˆss‰
...
if ($tee == "W") {
  ...
  if ($thyllyalue == "" or $thyllynro == "" or $thyllyvali == "" or $thyllytaso == "") {
    echo "<font class='error'>".t("VIRHE: Syˆtetty varastopaikka ei ollut t‰ydellinen")."</font><br>";
    $tee = "V";
  }

  if (kuuluukovarastoon($thyllyalue, $thyllynro) == 0) {
    echo "<font class='error'>".t("VIRHE: Syˆtetty varastopaikka ei kuulu mihink‰‰n varastoon")." ($ahyllyalue-$ahyllynro)</font><br>";
    $tee = "V";
  }
  ...
  foreach ($tunnukset as $key => $arvo) {
    ...
    if (mysql_num_rows($result) == 0) {
      ...
      $query = "INSERT into tapahtuma set
                ...
                hyllyvali = '$thyllyvali',
                hyllytaso = '$thyllytaso',
                selite    = '".t("Lis‰ttiin tuotepaikka")." $thyllyalue $thyllynro $thyllyvali $thyllytaso',
                laatija   = '$kukarow[kuka]',
                laadittu  = now()";
      $result = mysql_query($query) or pupe_error($query);
    }

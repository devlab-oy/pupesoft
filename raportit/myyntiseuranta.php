	...
	if (isset($tee) and $tee == "lataa_tiedosto") {
		...
		if ($lopetus == "") {
			...
			echo "<table>
				...
				<tr>
				<th>",t("N‰yt‰ tuotteiden myyntihinnat"),"</th>
			...
			echo "<tr>";
			echo "<th>".t('N‰yt‰ myyj‰budjetit')."</th>";
			echo "<td colspan='3'><input type='checkbox' name='myyjabudjetti' {$myyjabudjettichk} /></td>";
			echo "<td class='back'>".t('Toimii vain jos listaat myyjitt‰in')."</td>";
			echo "</tr>";
					...
					if ($query != "") {
						...
						while ($row = mysql_fetch_assoc($result)) {
							...
							if ($myyjabudjetti != '') {
								...
								else if ($row['asiakasmyyj‰'] != 0 and !isset($row['tuotemyyj‰']) and !isset($row['myyj‰'])) {
									$myyja_query = "SELECT budjetti
														FROM kuka
														WHERE kuka.yhtio = '{$kukarow['yhtio']}'
														AND kuka.myyja = '{$row['asiakasmyyj‰']}'";
								}
								else if ($row['myyj‰'] != 0 and !isset($row['tuotemyyj‰']) and !isset($row['asiakasmyyj‰'])) {
									$myyja_query = "SELECT budjetti
														FROM kuka
														WHERE kuka.yhtio = '{$kukarow['yhtio']}'

...
if ($yhtiorow['kuvapankki_polku'] == '') {
  echo "<font class='error'>Kuvapankkia ei ole määritelty. Ei voida jatkaa!</font>";
  exit;
}
...
if (!is_writable($dirri)) {
  echo "<font class='error'>Kuvapankkiin ($dirri) ei ole määritelty kirjoitusoikeutta. Ei voida jatkaa!<br></font>";
  exit;
}
...
if ($tee == 'GO') {
  ...
  if ($kasittele_kuvat != "1" and $thumb_kuvat != "1" and $normaali_kuvat != "1" and $paino_kuvat != "1" and $muut_kuvat != "1") {
    echo "<font class='message'>Et valinnut mitään käsiteltävää!</font>";
    exit;
  }
  ...
  if (isset($kasittele_kuvat) and $kasittele_kuvat == '1') {
    ...
    echo "<br>";
    echo "<font class='message'>Käsitellään konvertoitavia kuvia:</font>";
    echo "<br><br>";
    ...
    foreach ($files as $file) {
      ...
      if ($size !== FALSE and $toiminto == 'kasittele' and $mtype == "image") {
        ...
        if (file_exists($file)) {
          ...
          // konvertoidaan normaali kuva ja siirretään normaali hakemistoon
  ...
  echo "<font class='message'>Päivitetään tuotekuvat järjestelmään:</font>";
  echo "<br><br>";

  // käydään läpi dirikka nyt uudestaan
  $files = listdir($dirri);
  ...
  foreach ($files as $file) {
    ...
    // jos saimme jonkun imagesizen, katsellaan, että se on ok
    if ($koko !== FALSE) {
      if ($toiminto == 'thumb' and (($kork > $thumb_size_y and $thumb_size_y > 0) or ($leve > $thumb_size_x and $thumb_size_x > 0))) {
        ...
        if ($thumbi != "") {
          ...
          echo " &raquo; <font class='error'>Ohitetaan thumb-kuva, koska resoluutio $leve x $kork on liian suuri ja skaalaus epäonnistui!</font><br>";
          continue;
        }
      }
      ...
      if ($toiminto == 'normaali' and (($kork > $normaali_size_y and $normaali_size_y > 0) or ($leve > $normaali_size_x and $normaali_size_x > 0))) {
        ...
        if ($normi != "") {
          ...
          echo " &raquo; <font class='error'>Ohitetaan normaali-kuva, koska resoluutio $leve x $kork on liian suuri ja skaalaus epäonnistui!</font><br>";
          continue;
        }
      }
    }
    ...
    if (file_exists($file)) {
      ...
      if ($filesize == 0) {
        echo " &raquo; <font class='error'>Ohitetaan kuva, koska tiedosto on tyhjä!</font><br>";
        continue;
      }
      ...
      if (mysql_num_rows($apuresult) > 0) {
        ...
        echo " &raquo; Lisättiin liitetiedosto tuotteelle ";
        ...
        echo " &raquo; <font class='error'>Ohitetaan kuva, koska kuvalle ei löytynyt tuotetta!</font><br>";
      }
    }
  }
  echo "<br>";
...
if ($tee == 'DUMPPAA' and $mitkadumpataan != '') {
  ...
  while ($row = mysql_fetch_array($result)) {
    ...
    elseif ($row["kayttotarkoitus"] == "MU") {
      ...
      echo "<font class='message'>Tuntematon käyttötarkoitus $row[kayttotarkoitus]!</font>";
      continue;
    }
    ...
    if (!is_writable($kokonimi)) {
      echo "<font class='error'>Hakemistolle $kokonimi ei ole määritelty kirjoitusoikeutta. Ei voida tallentaa kuvaa!</font><br>";
      continue;
    }
    ...
    if (!file_exists($kokonimi)) {
      ...
      if ($handle === FALSE) {
        echo "<font class='error'>Tiedoston $kokonimi kirjoitus epäonnistui!</font><br>";
      }
      else {
  ...
  if ($dellattuja > 0) {
    echo "<br><font class='message'>Poistettiin $dellattuja kuvaa järjestelmästä</font><br>";
  }

...
if ($tee == 'uusi_ekirje') {
  ...
  echo "<br><font class='ok'>eKirje lähetetty uudestaan asiakkaalle $asiakastiedot[nimi]!</font><br><br>";
}
...
if ($tee == "uusi_ekirjekierros") {
  ...
  while ($uek_row = mysql_fetch_assoc($uek_res)) {
    ...
    try {
      ...
      echo "<font class='ok'>eKirje lähetetty uudestaan asiakkaalle $asiakastiedot[nimi]!</font><br>";
    }
    catch (Exception $e) {
      $ekarhu_success = false;
      echo "<font class='error'>Ei voitu lähettää karhua eKirjeenä. Virhe: " . $e->getMessage() . "</font>";
    }

    $uek_lask++;
  }

  echo "<br><br><font class='ok'>Lähetettiin: $uek_lask eKirjettä!</font><br><br><br>";
}
...
echo "<form name='karhu_selaa' method='post'>
    ...
    <tr>
      ...
      <td class='back'><input type='submit' name='tee_kaikki' value='".t("Näytä kaikki avoimet")."'></td>
    </tr>
    </table>
    </form>";

if ($toim == "") {
  echo "<br><br><font class='head'>".t("Selaa maksukehotuskierroksia / uudelleenlähetä kierros asiakkaille")."</font><hr />";
  echo "<form name='karhu_selaa' method='post'>
    <input type='submit' name='tee_kiekat' value='".t("Näytä maksukehotuskierrokset")."'>
    </form><br>";
}
...
if ((isset($tee_hae) and $tee_hae != "") or (isset($tee_kaikki) and $tee_kaikki != "") or (isset($tee_kierros) and $tee_kierros != "")) {
  ...
  if (mysql_num_rows($res) > 0) {
    ...
    if ($toim == "TRATTA") {
      ...
      echo "<th>".t('Maksukehotuspvm')."<br>".t('Eräpäivä')."</th>";
      ...
      if ($toim == "TRATTA") {
        ...
        if (isset($ekirje_config) and is_array($ekirje_config)) {
          echo "<td class='back'>
              ...
              <input type='submit' value='".t("Lähetä eKirje uudestaan")."'>
              </form></td>";
        }
      }

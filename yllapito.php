	...
	if ($tunnus == 0 and $uusi == 0 and $errori == '') {
		...
		if ($toim != "yhtio" and $toim != "yhtion_parametrit" and $uusilukko == "") {
			...
			echo "' method = 'post'>
					...
					***************************************************************************************************************/
/*				
					$titleArray = array(); //Array with all the Field Names of the selected table e.g. id
					$tablesArray = array(); //Array with all the table names of the database pupesoft
					$keyArray = array(); //Array with all the Primary keys of each table
					$resultTables = mysql_list_tables("pupesoft");
					$num_rows = mysql_num_rows($resultTables); //Total amount of tables
					
					$tableT = 0;
					
					//For loop that gets all the Primary Keys of all the tables
					for ($i = 0; $i < $num_rows; $i++) 
					{
						$tablesArray[$i] = mysql_tablename($resultTables, $i);
						$query = "SELECT * from $tablesArray[$i]";
						$result = mysql_query($query) or pupe_error($query);
						$numberOfFields = mysql_num_fields($result);
					
						$f = 0;
						while ($f < $numberOfFields) 
						{
							$meta = mysql_fetch_field($result, $f);

							if($meta->primary_key)
							{
								$keyArray[$i] = $meta->name;
							}
					...
					//For loop that loops through all of the the tables
					for ($tableT = 0; $tableT < 117; $tableT++) //23 && 36 not
					{
						if($tablesArray[$tableT]=='karhu_lasku' || $tablesArray[$tableT]=='liitetiedostot')
						{
							continue;
						}
						...
						echo $query = "SELECT * FROM $tablesArray[$tableT]";
						echo "</br>";
						$result = mysql_query($query) or pupe_error($query);
					    $totalColums = mysql_num_fields($result); //total amount of records
						
						while ( mysql_field_name ($result,$total)) 
						{
							$titleArray[$total] = mysql_field_name ($result,$total);
							$total++;
						}
						
						while ($row = mysql_fetch_array($result))
						{
							$wrde = $row[$keyArray[$tableT]];

							for ($tel = 0; $tel < mysql_num_fields($result); $tel++)
							{
								$regex = "(([\304\344\326\366a-zA-Z0-9.=_&!\/]*+\s*)+)";
								// preg_match("/^$regex$/",'Hyväksyttävät EXT-tilaukset');
								//echo $row[$tel];
								//echo "</br>";
								if( !preg_match("/^$regex$/",$row[$tel]))
								{
									$newValueISO = mysql_real_escape_string (iconv('UTF-8', 'ISO-8859-1', $row[$tel]));
									$queryUpdate = "UPDATE $tablesArray[$tableT] SET $titleArray[$k] = '$newValueISO' WHERE $keyArray[$tableT] = $wrde";
									//echo "</br>";
									$resultUpdate = mysql_query($queryUpdate) or pupe_error($queryUpdate);	
								}

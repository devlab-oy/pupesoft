...
//tässä määritellään kenen kaikkien kalenterit näytetään
if (isset($kalen)) {
...
if($tee == "SYOTA") {
  ...
  $lisays .= "<tr><th colspan='2'>".t("Lisää tapahtuma").":</th></tr>";
  ...
  if ($toim == 'TYOMAARAYS_ASENTAJA' or $tyomaarays != '') {
    $lisays .= "<tr><td>".t("Työmääräys").":</td><td><input type text name='tyomaarays' value='$tyomaarays'>";
    $lisays .= "</td></tr>";
  }
...
for ($i=1; $i <= days_in_month($kuu, $year); $i++) {
  ...
  // tehdään rivinvaihto jos kyseessä on sunnuntai ja seuraava päivä on olemassa..
...
if (mysql_num_rows($result) > 0) {
  echo "<table width='100%'>";
  echo "<tr>";
  echo "<th colspan='4'>".t("Kokopäivän tapahtumat")."</th>";
  echo "</tr>";
...
// Näytetään päivän kalenteritapahtumat //
// käydään eka tämä päivä kerran läpi ja tutkitaan onko päällekkäisiä tapahtumia
...
while ($date != $whileloppu) {
  ...
  while ($row=mysql_fetch_assoc($result)) {
    //tutkitaan kuinka monta päällekkäistä on parhaimmillaan...
...
while ($kello_nyt != $whileloppu) {
  ...
  //kirjoitetaan tiedot tauluun (paitsi jos ollaa lisäämässä uutta tähän ja taulu oisi tyhjä)
  echo "<tr>";
  echo "<td nowrap width='10%'>$kello_nyt <a href='$PHP_SELF?valitut=".urlencode($valitut)."&kenelle=".urlencode($kenelle)."&tee=SYOTA&kello=$kello_nyt&year=$year&kuu=$kuu&paiva=$paiva&konserni=$konserni&toim=$toim&tyomaarays=$tyomaarays&lopetus=$lopetus'>".t("Lisää")."</a></td>";

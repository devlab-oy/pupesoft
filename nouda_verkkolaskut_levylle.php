<?php
	...
	// Näytetään toimittajan laskuja joille lisätietoja voi syöttää
	if ($tee == 'A') { 
...
//			exec($kasky);
	...
	//	Siirretään tietokantaan turvaan kaikki siirtämättömät!
	if($tee == "siirrasiirtamattomat") {
				...
				while ($laskurow=mysql_fetch_array($result)) {
					$ebid=$laskurow['ebid'];
					echo "<font class='message'>Käsitellään lasku {$laskurow["nimi"]} {$laskurow["summa"]} {$laskurow["valkoodi"]}</font><br>ebid: {$laskurow["ebid"]}<br";
...
//be test begin --------------------------------------
		...
		echo "<br><br><form action = '$PHP_SELF' method='post'>
						...
						<input type='Submit' value='".t("Siirrä KAIKKI noutamattomat laskut liittetiedostoiksi tietokantaan")."'>
						</form>";

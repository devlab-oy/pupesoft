...
class MagentoClient {
  ...
  public function lisaa_tuotepuu(array $dnstuotepuu) {
    ...
    //$parent_id = $this->_parent_id; // Magento kategorian tunnus, jonka alle kaikki tuoteryhmät lisätään - muuttuu tietenkin jokaisella alakategorialla mutta defaulttaa tähän
    ...
    foreach ($dnstuotepuu as $alakategoria) {
      ...
      try {
        ...
        // O-taso on vain tuotepuun nimi, 1-taso on "isäkategoriat" jotka laitetaan aina magenton pääkategoriatunnuksen alle
        ...
        if (!$this->findCategory($alakategoria['nimi'], $category_tree['children'])) {
          ...
          $this->log("Lisättiin kategoria {$alakategoria['nimi']}");
        }
      }
      catch (Exception $e) {
        $this->_error_count++;
        $this->log("Virhe! Kategoriaa {$alakategoria['nimi']} ei voitu lisätä", $e);
      }
    }

    $this->_category_tree = $this->getCategories();
    $this->log("$count alakategoriaa lisätty");

...
if ($asiakasid != "") {
  ...
  if (mysql_num_rows($asiakas_result) != 1) {
    echo "<font class='error'>".t("Asiakasta ei löytynyt").".</font>";
    exit;
  }
...
if ($tee == 'uusi') {
  ...
  if(empty($uusi_alkuryhma) or empty($uusi_loppuryhma)) {
    echo "<br><font class='error'>".t("Alkuryhmä tai loppuryhmä")." puuttuu</font><br/>";
    $tee = "alennustaulukko";
  }
  if (!is_numeric($uusi_alennus) and $uusi_alennus != "") {
    echo "<br><font class='error'>".t("Alennuksen täytyy olla numero").".</font><br/>";
    $tee = "alennustaulukko";
  }
  if ($uusi_alennus < 0 or $uusi_alennus > 100) {
    echo "<br><font class='error'>".t("Alennuksen täytyy olla välillä 0-100").".</font><br/>";
    $tee = "alennustaulukko";
  }
  if (!is_numeric($uusi_minkpl) and $uusi_minkpl != "") {
    echo "<br><font class='error'>".t("Minkpl täytyy olla numero").".</font><br/>";
    $tee = "alennustaulukko";
  }
}
...
if ($tee == 'lisaa') {
  foreach($alkuryhma as $i => $ryhma) {
    ...
    if (empty($alkuryhma[$i]) or empty($loppuryhma[$i])) {
      echo "<font class='error'>".t("Alkuryhmä tai loppuryhmä puuttuu")."</font><br/>";
      $tee = "alennustaulukko";
    }
    if (!is_numeric($alennus[$i]) and $alennus[$i] != "") {
      echo "<font class='error'>".t("Alennuksen täytyy olla numero").".</font><br/>";
      $tee = "alennustaulukko";
    }
    if ($alennus[$i] < 0 or $alennus[$i] > 100) {
      echo "<font class='error'>".t("Alennuksen täytyy olla välillä 0-100").".</font><br/>";
      $tee = "alennustaulukko";
    }
    if (!is_numeric($minkpl[$i]) and $minkpl[$i] != "") {
      echo "<font class='error'>".t("Minkpl täytyy olla numero").".</font><br/>";
      $tee = "alennustaulukko";
    }
  }
}
...
if ($tee == 'lisaa' or $tee == 'uusi') {
  ...
  for ($i = 0; $i < count($alkuryhma); $i++) {
    ...
    # Jos ryhmiä ei löydy
    if (count($ryhmat) == 0) {
      echo "<br><font class='error'>".t("Yhtään ryhmää ei löytynyt väliltä")." $alkuryhma[$i] - $loppuryhma[$i]!</font><br>";
      $tee = "alennustaulukko";
      break;
    }
...
if ($tee == "alennustaulukko") {
  ...
  do {
    ...
    if ($edellinen_rivi != "" and ($edellinen_rivi['alennus'] != $row['alennus']
      ...
      if(empty($edellinen_rivi['monikerta'])) {
        ...
        echo "<option value='K' selected>".t("Kyllä")."</option>";
      }
      echo "</td>";
  ...
  echo "<td><select name='uusi_monikerta'>
        ...
        <option value='K' $sel>".t("Kyllä")."</option>
    </td>

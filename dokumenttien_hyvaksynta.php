...
if ($tee == "palauta") {
  ...
  if (trim($viesti) == "" or (int) $hyvaksyja == 0) {
    ...
    // Pakataan array stringiin, ett‰ saadaan se n‰tisti l‰pi
    ...
    if ((int) $hyvaksyja > 0) {
      ...
      // Hajotetaan stringi takaisin arrayksi k‰sittely‰ varten
      $hyvaksyneet_hyvaksyjat = explode (" ", $hyvaksyneet_hyvaksyjat);

      // Katsotaan ketk‰ ovat nykyisen hyv‰ksyj‰n ja valitun hyv‰ksyj‰n v‰liss‰ hyv‰ksyneet ja nollataan myˆs heid‰n hyv‰ksymisens‰, 
      // jotta saadaan dokumentti k‰ym‰‰n hyv‰ksymisketju tavalliseen tapaan l‰pi
  ...
  if ($onko_eka_hyvaksyja and $dokumenttirow['hyvaksynnanmuutos'] != '') {
    ...
    if ($dokumenttirow["h1time"] != "0000-00-00 00:00:00") {
      echo "<form method='post'>
            ...
            <input type='hidden' name='tunnus' value='$tunnus'>
            <input type='submit' value='".t("Palauta dokumentti edelliselle hyv‰ksyj‰lle")."'>
            </form><br>";
    }

<?php
	...
	if (strpos($_SERVER['SCRIPT_NAME'], "mt_yllapito.php")  !== FALSE) {
	...
	for ($i=0; $i<=$count; $i++) {
		if (isset($haku[$i]) and strlen($haku[$i]) > 0) {
			...
			elseif ($from == "mt_yllapito" and ($toim == 'rahtisopimukset' or $toim == 'asiakasalennus' or $toim == 'asiakashinta') and trim($array[$i]) == 'asiakas') {
	...
	if ($tunnus == 0 and $uusi == 0 and $errori == '') {
		...
		if ($toim != "yhtio" and $toim != "yhtion_parametrit" and $uusilukko == "") {
			echo "	<form action = 'mt_yllapito.php?ojarj=$ojarj$ulisa";
		...
		if (mysql_num_rows($result) >= 350) {
			echo "	<form action = 'mt_yllapito.php?ojarj=$ojarj$ulisa' method = 'post'>
					<input type = 'hidden' name = 'toim' value = '$aputoim'>
					<input type = 'hidden' name = 'lopetus' value = '$lopetus'>
					<input type = 'hidden' name = 'js_open_yp' value = '$js_open_yp'>
		...
		if ($toim == "tuote" and $uusi != 1 and $errori == '' and $tmp_tuote_tunnus > 0) {
			...
			echo "<form action = 'mt_yllapito.php' method = 'post'>";
		...
		echo "	<br><br><table><tr class='aktiivi'>
				<form action='mt_yllapito.php?ojarj=$ojarj$ulisa' method='post'>
				<input type = 'hidden' name = 'toim' value = '$aputoim'>
				<input type = 'hidden' name = 'lopetus' value = '$lopetus'>
				<input type = 'hidden' name = 'js_open_yp' value = '$js_open_yp'>
				<input type = 'hidden' name = 'limit' value = '$limit'>
		...
		elseif ($from == "") {
			for ($i = 1; $i < mysql_num_fields($result); $i++) {
				if (strpos(strtoupper(mysql_field_name($result, $i)), "HIDDEN") === FALSE) {
					...
					echo "<th valign='top'><a href='mt_yllapito.php?toim=$aputoim&lopetus=$lopetus&ojarj=".($i+1)."_".$edosuu."$ulisa&limit=$limit&nayta_poistetut=$nayta_poistetut&nayta_eraantyneet=$nayta_eraantyneet&laji=$laji'>" . t(mysql_field_name($result,$i)) . "</a>";
		...
		if (($toim == "asiakasalennus" or $toim == "asiakashinta" or $toim == "hinnasto" or $toim == "puun_alkio") and $oikeurow['paivitys'] == 1) {
			echo "<tr><form action='mt_yllapito.php?ojarj=$ojarj$ulisa' name='ruksaus' method='post' onSubmit = 'return verifyMulti()'>
			...
			for ($i=1; $i < mysql_num_fields($result); $i++) {
				if (strpos(strtoupper(mysql_field_name($result, $i)), "HIDDEN") === FALSE) {
					if ($i == 1) {
						...
						echo "<td valign='top'><a name='$trow[0]' href='mt_yllapito.php?ojarj=$ojarj$ulisa&toim=$aputoim&tunnus=$trow[0]&limit=$limit&nayta_poistetut=$nayta_poistetut&nayta_eraantyneet=$nayta_eraantyneet&laji=$laji";

						if ($lopetus == "") {
							echo "&lopetus=".$palvelin2."mt_yllapito.php////ojarj=$ojarj".str_replace("&", "//", $ulisa)."//toim=$aputoim//limit=$limit//nayta_poistetut=$nayta_poistetut//nayta_eraantyneet=$nayta_eraantyneet//laji=$laji///$trow[0]";
						}
						else {
	...
	if ($tunnus > 0 or $uusi != 0 or $errori != '') {
		...
		echo "<form action = 'mt_yllapito.php?ojarj=$ojarj$ulisa$ankkuri' name='mainform' id='mainform' method = 'post' autocomplete='off' $javalisasubmit enctype='multipart/form-data'>";
		echo "<input type = 'hidden' name = 'toim' value = '$aputoim'>";
		echo "<input type = 'hidden' name = 'js_open_yp' value = '$js_open_yp'>";
		echo "<input type = 'hidden' name = 'limit' value = '$limit'>";
		echo "<input type = 'hidden' name = 'nayta_poistetut' value = '$nayta_poistetut'>";
		...
		echo "</form>";
		if ($errori == '' and $toim == "avainsana" and $from != "mt_yllapito" and $from != "positioselain") {
		...
		if ($trow["tunnus"] > 0 and $errori == '' and $toim == "lasku") {
			...
			echo "<iframe id='laskun_lisatiedot_iframe' name='laskun_lisatiedot_iframe' src='mt_yllapito.php?toim=laskun_lisatiedot&from=mt_yllapito&ohje=off&haku[1]=$trow[tunnus]&lukitse_avaimeen=$trow[tunnus]' style='width: 600px; border: 0px; display: block;' border='0' frameborder='0'></iFrame>";
		}

		if ($trow["tunnus"] > 0 and $errori == '' and $toim == "asiakas") {

			$queryoik = "SELECT tunnus from oikeu where nimi like '%mt_yllapito.php' and alanimi = 'asiakasalennus' and kuka = '$kukarow[kuka]' and yhtio = '$yhtiorow[yhtio]'";
			$res = mysql_query($queryoik) or pupe_error($queryoik);
				
			if (mysql_num_rows($res) > 0) echo "<iframe id='asiakasalennus_iframe' name='asiakasalennus_iframe' src='mt_yllapito.php?toim=asiakasalennus&from=mt_yllapito&ohje=off&haku[1]=$trow[tunnus]/$trow[ytunnus]&lukitse_avaimeen=$trow[tunnus]' style='width: 600px; border: 0px; display: block;' border='0' frameborder='0'></iFrame>";

			$queryoik = "SELECT tunnus from oikeu where nimi like '%mt_yllapito.php' and alanimi = 'asiakashinta' and kuka = '$kukarow[kuka]' and yhtio = '$yhtiorow[yhtio]'";
			$res = mysql_query($queryoik) or pupe_error($queryoik);

			if (mysql_num_rows($res) > 0) echo "<iframe id='asiakashinta_iframe' name='asiakashinta_iframe' src='mt_yllapito.php?toim=asiakashinta&from=mt_yllapito&ohje=off&haku[1]=$trow[tunnus]/$trow[ytunnus]&lukitse_avaimeen=$trow[tunnus]' style='width: 600px; border: 0px; display: block;' border='0' frameborder='0'></iFrame>";

			$queryoik = "SELECT tunnus from oikeu where nimi like '%mt_yllapito.php' and alanimi = 'asiakaskommentti' and kuka = '$kukarow[kuka]' and yhtio = '$yhtiorow[yhtio]'";
			$res = mysql_query($queryoik) or pupe_error($queryoik);

			if (mysql_num_rows($res) > 0) echo "<iframe id='asiakaskommentti_iframe' name='asiakaskommentti_iframe' src='mt_yllapito.php?toim=asiakaskommentti&from=mt_yllapito&ohje=off&haku[1]=$trow[ytunnus]&lukitse_avaimeen=$trow[ytunnus]' style='width: 600px; border: 0px; display: block;' border='0' frameborder='0'></iFrame>";

			$queryoik = "SELECT tunnus from oikeu where nimi like '%mt_yllapito.php' and alanimi = 'asiakkaan_avainsanat' and kuka = '$kukarow[kuka]' and yhtio = '$yhtiorow[yhtio]'";
			$res = mysql_query($queryoik) or pupe_error($queryoik);

			if (mysql_num_rows($res) > 0) echo "<iframe id='asiakkaan_avainsanat_iframe' name='asiakkaan_avainsanat_iframe' src='mt_yllapito.php?toim=asiakkaan_avainsanat&from=mt_yllapito&ohje=off&haku[4]=$trow[tunnus]&lukitse_avaimeen=$trow[tunnus]' style='width: 600px; border: 0px; display: block;' border='0' frameborder='0'></iFrame>";

			$queryoik = "SELECT tunnus from oikeu where nimi like '%mt_yllapito.php' and alanimi like 'puun_alkio&laji=asiakas%' and kuka = '$kukarow[kuka]' and yhtio = '$yhtiorow[yhtio]'";
			$res = mysql_query($queryoik) or pupe_error($queryoik);

			if (mysql_num_rows($res) > 0) echo "<iframe id='puun_alkio_iframe' name='puun_alkio_iframe' src='mt_yllapito.php?toim=puun_alkio&laji=asiakas&from=mt_yllapito&ohje=off&haku[1]=$trow[tunnus]&lukitse_avaimeen=$trow[tunnus]&toim2=asiakas' style='width: 600px; border: 0px; display: block;' border='0' frameborder='0'></iFrame>";
		...
		if ($trow["tunnus"] > 0 and $errori == '' and ($toim == "toimi" or $toim == "asiakas")) {
			...
			$queryoik = "SELECT tunnus from oikeu where nimi like '%mt_yllapito.php' and alanimi = 'yhteyshenkilo' and kuka = '$kukarow[kuka]' and yhtio = '$yhtiorow[yhtio]'";
			$res = mysql_query($queryoik) or pupe_error($queryoik);

			if (mysql_num_rows($res) > 0) echo "<iframe id='yhteyshenkilo_iframe' name='yhteyshenkilo_iframe' src='mt_yllapito.php?toim=yhteyshenkilo&from=mt_yllapito&laji=$laji&ohje=off&haku[6]=@$tunnus&lukitse_avaimeen=$tunnus' style='width: 600px; border: 0px; display: block;' border='0' frameborder='0'></iFrame>";
		}
		...
		if ($trow["tunnus"] > 0 and $errori == '' and ($toim == "toimitustapa" or $toim == "maksuehto")) {
			...
			$queryoik = "SELECT tunnus from oikeu where nimi like '%mt_yllapito.php' and alanimi = 'avainsana' and kuka = '$kukarow[kuka]' and yhtio = '$yhtiorow[yhtio]'";
			$res = mysql_query($queryoik) or pupe_error($queryoik);

			if (mysql_num_rows($res) > 0) echo "<iframe id='avainsana_iframe' name='avainsana_iframe' src='mt_yllapito.php?toim=avainsana&from=mt_yllapito&lukitse_laji=$laji&ohje=off&haku[2]=@$laji&haku[3]=@$tunnus&lukitse_avaimeen=$tunnus' style='width: 600px; border: 0px; display: block;' border='0' frameborder='0'></iFrame>";
		}
		...
		if ($trow["tunnus"] > 0 and $errori == '' and ($toim == 'lasku' or $toim == 'asiakas' or $toim == "sarjanumeron_lisatiedot" or ($toim == "tuote" and $laji != "V") or (($toim == "avainsana") and (strtolower($laji) == "osasto" or strtolower($laji) == "try" or strtolower($laji) == "tuotemerkki")))) {
			$queryoik = "SELECT tunnus from oikeu where nimi like '%mt_yllapito.php' and alanimi='liitetiedostot' and kuka='$kukarow[kuka]' and yhtio='$yhtiorow[yhtio]'";
			$res = mysql_query($queryoik) or pupe_error($queryoik);

			if (mysql_num_rows($res) > 0) echo "<iframe id='liitetiedostot_iframe' name='liitetiedostot_iframe' src='mt_yllapito.php?toim=liitetiedostot&from=mt_yllapito&ohje=off&haku[7]=@$toim&haku[8]=@$tunnus&lukitse_avaimeen=$tunnus&lukitse_laji=$toim' style='width: 600px; border: 0px; display: block;' border='0' frameborder='0'></iFrame>";
		}

		if ($trow["tunnus"] > 0 and $errori == "" and $from != "mt_yllapito" and $toim == "tuote" and $laji != "V") {

			$lukitse_avaimeen = urlencode($tuoteno);

			$queryoik = "SELECT tunnus from oikeu where nimi like '%mt_yllapito.php' and alanimi='tuotteen_toimittajat' and kuka='$kukarow[kuka]' and yhtio='$yhtiorow[yhtio]'";
			$res = mysql_query($queryoik) or pupe_error($queryoik);

			if (mysql_num_rows($res) > 0) echo "<iframe id='tuotteen_toimittajat_iframe' name='tuotteen_toimittajat_iframe' src='mt_yllapito.php?toim=tuotteen_toimittajat&from=mt_yllapito&ohje=off&haku[1]=@$lukitse_avaimeen&lukitse_avaimeen=$lukitse_avaimeen' style='width: 600px; border: 0px; display: block;' border='0' frameborder='0'></iFrame>";

			$queryoik = "SELECT tunnus from oikeu where nimi like '%mt_yllapito.php' and alanimi='tuotteen_avainsanat' and kuka='$kukarow[kuka]' and yhtio='$yhtiorow[yhtio]'";
			$res = mysql_query($queryoik) or pupe_error($queryoik);

			if (mysql_num_rows($res) > 0) echo "<iframe id='tuotteen_avainsanat_iframe' name='tuotteen_avainsanat_iframe' src='mt_yllapito.php?toim=tuotteen_avainsanat&from=mt_yllapito&ohje=off&haku[1]=@$lukitse_avaimeen&lukitse_avaimeen=$lukitse_avaimeen' style='width: 600px; border: 0px; display: block;' border='0' frameborder='0'></iFrame>";

			$queryoik = "SELECT tunnus from oikeu where nimi like '%mt_yllapito.php' and alanimi like 'puun_alkio&laji=tuote%' and kuka='$kukarow[kuka]' and yhtio='$yhtiorow[yhtio]'";
			$res = mysql_query($queryoik) or pupe_error($queryoik);

			if (mysql_num_rows($res) > 0) echo "<iframe id='puun_alkio_iframe' name='puun_alkio_iframe' src='mt_yllapito.php?toim=puun_alkio&laji=tuote&from=mt_yllapito&ohje=off&haku[1]=@$lukitse_avaimeen&lukitse_avaimeen=$lukitse_avaimeen&toim2=tuote' style='width: 600px; height: 600px; border: 0px; display: block;' border='0' frameborder='0'></iFrame>";
		}
		...
		if ($toim == "puun_alkio" or
		   ...
		   ($toim == "toimitustapa" and $poistolukko == "") or
			...
			if ($uusi != 1 and $toim != "yhtio" and $toim != "yhtion_parametrit") {
				...
				if ($rajattu_nakyma == '') {
					echo "<br><br>
						<form action = 'mt_yllapito.php?ojarj=$ojarj$ulisa' method = 'post' onSubmit = 'return verify()' enctype='multipart/form-data'>
						<input type = 'hidden' name = 'toim' value = '$aputoim'>
						<input type = 'hidden' name = 'js_open_yp' value = '$js_open_yp'>
						<input type = 'hidden' name = 'limit' value = '$limit'>
						<input type = 'hidden' name = 'nayta_poistetut' value = '$nayta_poistetut'>
	...
	elseif ($toim != "yhtio" and $toim != "yhtion_parametrit"  and $uusilukko == "" and $from == "") {
		echo "<br>
				<form action = 'mt_yllapito.php?ojarj=$ojarj$ulisa";
				if ($liitostunnus) echo "&liitostunnus=$liitostunnus";
				echo "' method = 'post'>
				<input type = 'hidden' name = 'toim' value = '$aputoim'>
				<input type = 'hidden' name = 'js_open_yp' value = '$js_open_yp'>

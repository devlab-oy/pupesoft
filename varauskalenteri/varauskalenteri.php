...
echo "  <table align='left' width='50%'>
    ...
    <a href='$PHP_SELF?day=$backdday&month=$backmday&year=$backyday&toim=$toim'><< ".t("Edellinen päivä")."</a>&nbsp;&nbsp;&nbsp;
    <b>$day. ". $MONTH_ARRAY[$month] ." $year</b>&nbsp;&nbsp;&nbsp;
    <a href='$PHP_SELF?day=$nextdday&month=$nextmday&year=$nextyday&toim=$toim'>".t("Seuraava päivä")." >></a>
    </tr>
    <tr>
    <th align='left' width='100'>".t("Kello") ."</th>
    <th align='left' width='80'>" .t("Lisää") ."</th>
...
for($i=800; $i < 2200; $i++) {
  ...
  if(mysql_num_rows($result) != 0) {
    ...
    echo "<a href='$PHP_SELF?tee=NAYTA&year=$year&month=$month&day=$day&tunnus=$row[tunnus]&toim=$toim'>".t("Lisätiedot")."</a>";
    ...
    if($tun.$min <= 2100) {
      echo "<a href='$PHP_SELF?kello=$kello&year=$year&month=$month&day=$day&tee=SYOTA&toim=$toim'>".t("Lisää")."</a>";
    }
    echo "  </td>
        <td></td>
        <td colspan='2'></td>
...
// kirjoitetaan päivämäärät taulukkoon..

...
function viidakko_toimita_tilaus($params) {
  global $viidakko_url, $viidakko_webhooks_key;

  // jos ViidakkoStore API ei k‰ytˆss‰
  if (empty($viidakko_url)
      or empty($viidakko_webhooks_key)) {
    return false;
  }
  ...
  $viidakko_tilausnumerot = viidakko_hae_tilausnumerot($pupesoft_tunnukset);

  foreach ($viidakko_tilausnumerot as $order_number) {

    // Webhooks osoite
    $url = "{$viidakko_url}/webhooks/process";
...
function viidakko_hae_tilausnumerot($pupesoft_tunnukset) {
  ...
  pupesoft_log("viidakko_toimita_tilaus", "P‰ivitet‰‰n tilaukset $tunnukset toimitetuiksi.");

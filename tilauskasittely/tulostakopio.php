			...
			while ($row = mysql_fetch_assoc($result)) {
				...
				if ($tila != 'monta') {
					...
					echo "<form id='tulostakopioform_$row[tunnus]' name='tulostakopioform_$row[tunnus]' method='post' action='$PHP_SELF' autocomplete='off'>
							...
							<input type='hidden' name='mista' value='tulostakopio'>
							<input type='submit' value='".t("Näytä pdf")."' onClick=\"js_openFormInNewWindow('tulostakopioform_$row[tunnus]', 'samewindow'); return false;\"></form>";  // Muokattu 8.10.2013, vaihdettu parametri tulostakopio_$row[tunnus] -> samewindow (näin estetään tyhjän popupin ilmestyminen)
		...
		while ($laskurow = mysql_fetch_assoc($rrrresult)) {
			...
			if ($toim == "SAD") {
				...
				if ($yhtiorow["sad_lomake_tyyppi"] == "T" and $tee != 'NAYTATILAUS') {
					...
					$ruutu_nimi = substr($pdffilenimi,7);  // Lisätty 8.10.2013
					...
					elseif ($tee == 'NAYTATILAUS') {
						...
						/* Lisätty 8.10.2013, tällätavoin käyttäjä saa valita mitä pdf tiedostolle tekee (ja näin ne saa näkymään kaikilla) */

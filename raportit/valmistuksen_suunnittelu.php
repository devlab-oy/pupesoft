<?php
	...
	function teerivi($tuoteno, $isatuoteno, $kerroin) {
		...
		$valmistussuositus = ceil($valmistussuositus / $toimittajan_pakkauskoko) * $toimittajarow['pakkauskoko'] ; // Pyˆristet‰‰n pakkauskoon perusteella
	...
	if ($tee == "RAPORTOI" and isset($ehdotusnappi)) {
		...
		$query = "	SELECT lasku.kohde valmistuslinja,
					tilausrivi.tuoteno,
					tilausrivi.osasto,
					tilausrivi.try,
		...
		if (@mysql_num_rows($res)>0) {
			...
			echo t('Ei valmistuksia haetulla aikav‰lill‰')."<br>\n";
		}

...
if ($tee == '') {
  ...
  if ($kukarow["kesken"] > 0 and (
    ...
    if (mysql_num_rows($meapu) == 1 and $toimitustapa != '') {
      ...
      if ($meapurow["kateinen"] != "") {
        ...
        if ($meapu2row["nouto"] == "" and ($laskurow['tilaustyyppi'] != "W" and empty($_etukateen_maksettu))) {
          $apuqu = "SELECT *
                    FROM toimitustapa
                    WHERE yhtio  = '$kukarow[yhtio]'
                    AND nouto   != ''
  ...
  if (!empty($_etukateen_maksettu)) {
    ...
    echo "<br><font class='error'>",t("HUOM: Tilaus on etukäteen maksettu verkkokauppatilaus. Tilauksen rivitietoja ei voi muuttaa. Tee mahdolliset korjaukset tekemällä uusi tilaus."),"</font><br>";
  }
  ...
  if ((int) $kukarow["kesken"] > 0) {
    ...
    if ($rivilaskuri > 0) {
      ...
      if ($kukarow['hinnat'] != -1 and $toim != "SIIRTOTYOMAARAYS" and $toim != "VALMISTAVARASTOON") {
        ...
        if ($kukarow["extranet"] == "" and empty($_etukateen_maksettu)) {
  ...
  if ((int) $kukarow["kesken"] > 0 and (!isset($ruutulimit) or $ruutulimit == 0)) {
    ...
    if (($muokkauslukko == "" or $toim == "PROJEKTI" or $toim == "YLLAPITO") and $laskurow["liitostunnus"] != 0 and $tilausok == 0 and $rivilaskuri > 0 and $asiakasOnProspekti != "JOO") {
      ...
      elseif ($kukarow['tilaus_valmis'] != "4" and $yhtiorow['laite_huolto'] == '' and ($toim != 'REKLAMAATIO' or
          ($yhtiorow['reklamaation_kasittely'] == '' or
            ($yhtiorow['reklamaation_kasittely'] == 'X' and $laskurow['tilaustyyppi'] == 'U')) )) {
    ...
    if (($muokkauslukko == "" or $myyntikielto != '') and ($toim != "PROJEKTI" or ($toim == "PROJEKTI" and $projektilask == 0)) and $kukarow["mitatoi_tilauksia"] == "" and $ei_laskutettu and $ei_valmistettuand and $yhtiorow['laite_huolto'] == '') {
      echo "<SCRIPT LANGUAGE=JAVASCRIPT>
            function verify(){

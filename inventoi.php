		...
		if (count($tuote) > 0) {
			foreach($tuote as $i => $tuotteet) {
				...
				if ($kpl != '') {
					...
					if ($onko_vanhoja > 0 and $onko_uusia > 0) {
						echo "<font class='error'>".t("VIRHE: Voit lisätä / poistaa vain uuden sarjanumeron")."!</font><br>";
						$virhe = 1;
					}

					//Sarjanumerot
					if (is_array($sarjanumero_kaikki[$i]) and substr($kpl,0,1) != '+' and substr($kpl,0,1) != '-' and ($onko_uusia > 0 or $hyllyssa[$i] < $kpl)) {
						...
						if (($lista != '' and $row["inventointilista_aika"] != "0000-00-00 00:00:00") or ($lista == '' and $row["inventointilista_aika"] == "0000-00-00 00:00:00") or ($lista != '' and $tee2 == 'KORJAA')) {
							...
							elseif ($tee2 == "KORJAA" and $row["sarjanumeroseuranta"] == "") {
								...
								if ($tilidaterow["tapvm"] > $yhtiorow["tilikausi_alku"]) {
									...
									if ($uuserotus > 0) {
										$query .= " '".t("Saldoa")." ($nykyinensaldo) ".t("paikalla")." $hyllyalue-$hyllynro-$hyllyvali-$hyllytaso ".t("lisättiin")." $uuserotus ".t("kappaleella. Saldo nyt")." $cursaldo. <br>$lisaselite<br>$inven_laji'";
									}
									elseif ($uuserotus < 0) {
										$query .= " '".t("Saldoa")." ($nykyinensaldo) ".t("paikalla")." $hyllyalue-$hyllynro-$hyllyvali-$hyllytaso ".t("vähennettiin")." ".abs($uuserotus)." ".t("kappaleella. Saldo nyt")." $cursaldo. <br>$lisaselite<br>$inven_laji'";
									}
									else {
										$query .= " '".t("Saldo")." ($nykyinensaldo) ".t("paikalla")." $hyllyalue-$hyllynro-$hyllyvali-$hyllytaso ".t("täsmäsi.")." <br>$lisaselite<br>$inven_laji'";
									}
			...
			while($tuoterow = mysql_fetch_array($saldoresult)) {
				...
				if ($tuoterow["sarjanumeroseuranta"] != "") {
					$query = "	SELECT sarjanumeroseuranta.sarjanumero, sarjanumeroseuranta.tunnus, tilausrivi_myynti.otunnus myyntitunnus, tilausrivi_myynti.varattu myyntikpl,
								...
								and ((tilausrivi_myynti.tunnus is null or tilausrivi_myynti.laskutettuaika = '0000-00-00') and (tilausrivi_osto.laskutettuaika != '0000-00-00' or tilausrivi_osto.laskutettuaika is null or (tilausrivi_osto.laatija = 'Invent' and tilausrivi_osto.laskutettuaika = '0000-00-00')))
				...
				if (($tuoterow["inventointilista_aika"] == '0000-00-00 00:00:00' and $lista == '') or ($tuoterow["inventointilista"] == $lista and $tuoterow["inventointilista_aika"] != '0000-00-00 00:00:00') or ($tee2=='KORJAA' and $tuoterow["inventointilista"] == $lista)) {
					...
					if ($tuoterow["sarjanumeroseuranta"] == "S" or $tuoterow["sarjanumeroseuranta"] == "U" or $tuoterow['sarjanumeroseuranta'] == 'T' or $tuoterow['sarjanumeroseuranta'] == 'V') {
						if (mysql_num_rows($sarjares) > 0) {
							...
							while($sarjarow = mysql_fetch_array($sarjares)) {
								...
								echo "<tr><td>
										$sarjalaskk. $sarjarow[sarjanumero]</td><td align='right'>";
...
//							echo "<tr><td>".t("Ruksaa kaikki")."</td><td align='right'><input type='checkbox' onclick='toggleAll(this, \"sarjanumero_valitut[$tuoterow[tptunnus]][]\");'></td></tr>";
							echo "</table>";
						}
					...
					elseif ($tuoterow["sarjanumeroseuranta"] == "G" or $tuoterow["sarjanumeroseuranta"] == "E" or $tuoterow["sarjanumeroseuranta"] == "F") {
						if (mysql_num_rows($sarjares) > 0) {
							echo "<br><table width='100%'>";

			...
			if (mysql_num_rows($res) > 0) {
				...
				if ($yhtiorow["lisakulu_tuotenumero"] != "") {
					...
					foreach ($yhdista as $otsikot) {
						...
						// lis‰t‰‰n n‰ille tilauksille toimitustavan lisakulut
						$virhe = 0;
						...
						if (mysql_num_rows($otsre) == 1 and $virhe == 0) {
							...
							if ($ekrow['lisakulu_summa'] != 0 and $ekrow['lisakulu'] != 0) {
								...
								// jos tuotenumero lˆytyy lisakulun rajasumma ei tayty
								if (mysql_num_rows($rhire) == 1 and ($tilausrivit_kokosumma < $ekrow['lisakulu_summa'])) {
									...
									$nimitys = t("Toimitustavan lis‰kulu", $laskun_kieli);
									$kommentti = "";
									...
									if ($silent == "") {
										$tulos_ulos .= "<tr><td>".t("Lis‰ttiin toimitustavan lis‰kulut")."</td><td>{$laskurow['tunnus']}</td><td>{$laskurow['toimitustapa']}</td><td>{$tlhinta}</td><td>{$yhtiorow['valkoodi']}</td></tr>\n";
									}
								}
							}
						}

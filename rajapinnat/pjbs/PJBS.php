...
class PJBS {
  ...
  function __construct($jdbc_enc, $app_enc) {
    ...
    // 10 sec timeout
    $this->sock = fsockopen($conf[0], $conf[1], $sock_con_errno, $sock_con_errmsg, 10);

    if ($this->sock === false) {
      echo "Error connecting {$conf[0]}:{$conf[1]}: {$sock_con_errno} {$sock_con_errmsg}";
      die;
    }

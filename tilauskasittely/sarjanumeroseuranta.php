<?php
	...
	if (mysql_error() == "") {
		...
		//	Tutkitaan onko meill‰ venelis‰tiedot vai ei..
	...
	if ($from != '' and $rivitunnus != "" and $formista == "kylla") {
		...
		elseif (($rivirow["varattu"] < count($sarjataan) and $rivirow["sarjanumeroseuranta"] != "G") or ($rivirow["sarjanumeroseuranta"] == "G" and ($rivirow["varattu"] == 0 or count($sarjataan) == 0))) {
		...
		if ((($rivirow["varattu"] >= count($sarjataan) and count($sarjataan) > 0) or($rivirow["sarjanumeroseuranta"] == "G" and $rivirow["varattu"] > 0 and count($sarjataan) > 0)) and $lisaysok == "OK") {
		...
		while ($sarjarow = mysql_fetch_assoc($sarjaresiso)) {
			...
			if (($sarjarow["siirtorivitunnus"] > 0 and $tunnuskentta != 'siirtorivitunnus') or ($sarjarow["osto_perheid2"] > 0 and $sarjarow["osto_perheid2"] != $sarjarow["osto_rivitunnus"])) {
				...
				if ($siirow["tyyppi"] == "O") {
					...
					$fnlina1 .= "";
					$fnlina2 .= "<br><br>".t("Varattu lis‰varusteena").":<br>".$siirow["tuoteno"]." <a href='".$palvelin2."tilauskasittely/sarjanumeroseuranta.php?tuoteno_haku=".urlencode($siirow["tuoteno"])."&sarjanumero_haku=".urlencode($siirow["sarjanumero"])."'>$siirow[sarjanumero]</a>";
				}
			...
			if ($sarjanumeronLisatiedot == "OK") {
				...
				echo "<br><a href='#' onClick=\"javascript:sarjanumeronlisatiedot_popup('$sarjarow[tunnus]')\">".t("Lis‰tietoikkuna")."</a>";
			}

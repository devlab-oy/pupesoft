...
elseif(in_array($valitsetoimitus,array("TARJOUS","PIKATILAUS","VALMISTAASIAKKAALLE","SIIRTOLISTA","TYOMAARAYS", "REKLAMAATIO"))) {
...
if ($tee == '') {
	...
	if ((int) $kukarow["kesken"] != 0) {
		...
		if ($rivilaskuri != 0) {
			...
			if ($toim == "VALMISTAVARASTOON") {
				echo "<tr><td class='back' colspan='10'><font class='message'>";
			...
			while ($row = mysql_fetch_array($result)) {
				...
				elseif ($toim == "MYYNTITILI" and $laskurow["alatila"] == "V" and ($row["perheid"] == 0 or $row["perheid"] == $row["tunnus"])) {
					...
					if ($kpl_ruudulle < 0 and $row["sarjanumeroseuranta"] != "") {
	...
	if ((int) $kukarow["kesken"] != 0) {
		...
		if ($lopetus != "") {
			...
			echo "<tr><td class='back'><form name='lopetusformi' action='$lopetus' method='post'>

			...
			if ($toimittajaid != "") {
				...
				if (mysql_num_rows($result) > 0) {
					...
					if (in_array($laskurow["vienti"], array("B","E","H"))) {
						...
						echo "<table><tr><th>".t("Saapuminen")."</th><th>".t("Kommentit")."</th><th>".t("Rivejä")."/".t("varasossa")."</th><th>".t("Varastonarvo")."</th><th>".t("Summa")."</th></tr>";
						...
						echo "<tr><td class='back' colspan='5' align = 'right'><input type='submit' value='".t("Liitä saapumisille")."'></td></tr></table></form>";
					}
					else {
						...
						while($row = mysql_fetch_assoc($result)) {
							...
							echo "<td class='back' colspan='5' align = 'right'>
									...
									<input type='submit' value='".t("Liitä saapuminen")."'>
									</form>";
							echo "</td></tr>";
						}
						echo "</table>";
	...
	if (strlen($tunnus) != 0) {
		...
		if (in_array($laskurow["vienti"], array("B","E","H","C","J","F","K","I","L"))) {
			...
			if ($jaljella > 0) {
				...
				$lisa = "<a id='uusi' href='javascript:sndReq(\"keikka\", \"{$palvelin2}hyvak.php?keikalla=on&tee=haekeikka&tunnus=$tunnus$toimilisa\");'>".t("Liitä saapumiselle")."</a>";
			}
			else {
				$lisa = "";
			}

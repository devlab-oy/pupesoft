					...
					if ($srow1["sarjanumeroseuranta"] != "") {
						...
						if ($tunken == "myyntirivitunnus") {
							...
							if (mysql_num_rows($sarres) > 0 and $srow2["ei_ostohintaa"] > 0) {
								...
								if ($silent == "" or $silent == "VIENTI") {
									$tulos_ulos_sarjanumerot .= t("$query Olet myym‰ss‰ k‰ytetty‰ venett‰, jota ei ole viel‰ ostettu! Ei voida laskuttaa!").": $laskurow[tunnus] $srow1[tuoteno] $laskurow[nimi]!!!<br>\n";
								}
							}
						}
					}
			...
			if ($lask > 0) {
				...
				if ($yhtiorow['lasku_tulostin'] != 0 or (isset($valittu_tulostin) and $valittu_tulostin != "") or count($tulostettavat_email) > 0) {
					...
					foreach ($tulostettavat as $lasku) {
						...
						// jos t‰m‰ haluttiin suoraan asiakkaalle s‰hkˆpostiin
						if ($yhtiorow['lasku_tulostin'] == -99 or in_array($laskurow["laskunro"], $tulostettavat_email)) {
							...
							// Jos asiakkaalla on laskutuss‰hkˆposti tai jos $yhtiorow['lasku_tulostin'] != -99 niin silloin lasku menee kukarow[email]:iin jos $komento == ""

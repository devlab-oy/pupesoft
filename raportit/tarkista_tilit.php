...
echo "<br><font class='message'>Tiliöintien tilit</font><br>";
$query = "	SELECT distinct tili.tilino t, tiliointi.tilino
			...
			WHERE tiliointi.yhtio='$kukarow[yhtio]'  and korjattu='' and tiliointi.tilino > 0
			HAVING t IS NULL";
$result = mysql_query($query) or pupe_error($query);	
while($tili = mysql_fetch_array($result)) {
	$query = "	SELECT tapvm viimeisin, selite
				FROM tiliointi
				WHERE tiliointi.yhtio='$kukarow[yhtio]' and tiliointi.tilino = $tili[tilino] and korjattu=''
				ORDER BY tapvm asc
				LIMIT 1";
	$res = mysql_query($query) or pupe_error($query);	
	$viimrow = mysql_fetch_array($res);
	echo "<br><font class='error'>Tiliä $tili[tilino] ei ole enään olemassa! viimeisin tiliöinti $viimrow[viimeisin], $viimrow[selite]</font><br>";
}
		...
		while($col = mysql_fetch_array($res)) {
			if(in_array($col[0], $columnit)) {
				foreach($columnit as $c) {
					$query = "	SELECT $c
								FROM $table[0]
								WHERE yhtio = '$kukarow[yhtio]' and $c != ''";

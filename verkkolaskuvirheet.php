<?php
	...
	//Verkkolaskuille ei ole vakiopaikkaa! Anna tässä polut missä haluat laskuja säilyttää
	...
	if (isset($tiedosto)) {
		if ($tapa=='') {
			...
			for ($i=0; $i < mysql_num_fields($resultx) - 1; $i++) {
				...
				if (mysql_field_name($resultx, $i) == "tilinumero") {
					...
					if ((int) substr($t[$i],0,2) == 0) {  //Tuolla oli maa --> Tämä on iban
						$t[$i] = substr($t[$i],4); // Jätetään 4 ekaa pois.
					}
				}
		...
		if ($tapa == 'U') {
			...
			echo "<font class='message'>Tiedosto käsitellään uudestaan</font><br>";
		}
		 
		if ($tapa == 'P') {
			passthru("mv $vaarat/$tiedosto $poistetut/");
			echo "<font class='message'>Tiedosto hylättiin</font><br>";
		}
	}
	
	$laskuri = 0;
	$valitutlaskut = 0;
	echo "<font class='head'>".t("Hylätyt verkkolaskut")."</font><hr>";
				...
				if ($ok==1) {
					...
					if ($ok == 0) {
						if (mysql_num_rows($lahellaresult) > 0) {
							...
							echo "</select><input type='submit' value ='Päivitä toimittaja'></form>";
						}
						...
						<input type='submit' value ='Käsittele uudestaan'></form>";
					echo "<form action='$PHP_SELF' method='post'>
						<input type='hidden' name= 'tiedosto' value ='$file'>
						<input type='hidden' name= 'tapa' value ='P'>
						<input type='submit' value ='Hylkää'></form>";
					echo "</td>";
					...
					echo "<a href='$url'>". t('Näytä lasku')."</a>";
	...
	if ($valitutlaskut == 0) {
		echo "<font class='message'>Ei hylättyjä laskuja</font><br>";
	}
	echo $laskuri;
	require "inc/footer.inc";
?>

...
if ($tee == "keraa") {
  ...
  foreach ($kerattavat as $tilaus => $tuotteet) {
    ...
    if (mysql_num_rows($result) != 1) {
      echo "<font class='error'>".t("Tilausta: %s ei voida merkata kerätyksi. Tilaus väärässä tilassa", "", $tilaus)."!</font><br>";
      continue;
    }
    else {
    ...
    foreach ($tuotteet as $tuote => $kpl) {
      ...
      while ($valmis_era_chk_row = mysql_fetch_assoc($valmis_era_chk_res)) {
        ...
        if ($rivimaara == 1 and $valmis_era_chk_row['varattu'] != $kpl) {
          // Vain yksi rivi ja kerätty määrä on eri ku myyty määrä
          $maara[$valmis_era_chk_row['tunnus']] = $kpl;
        }
        elseif ($rivimaara > 1 and $rivimaara < $rivilask and $valmis_era_chk_row['varattu'] > $kpl) {
          // Enemmän kuin yksi rivi ja kerätty määrä on pienempi kuin myyty määrä
          $maara[$valmis_era_chk_row['tunnus']] = $kpl;
          $kpl = 0;
        }
        elseif ($rivimaara > 1 and $rivimaara == $rivilask and $valmis_era_chk_row['varattu'] != $kpl) {
          // Vika rivi ja kerätty määrä on eri kuin myyty määrä
          $maara[$valmis_era_chk_row['tunnus']] = $kpl;
        }
        else {
...
echo "<br><br>
    <font class='head'>".t("Lue kerättävät tuotteet tiedostosta")."</font><hr>
    <table>

				...
				while ($jtrow = mysql_fetch_assoc($isaresult)) {
					...
					if (($onko_suoratoimi == "" and $suoratoimit == "") or ($onko_suoratoimi == "ON" and $suoratoimit != "") or count($suoratoimitus_rivit) > 0) {
						...
						if ($kokonaismyytavissa > 0 or $toimi == '') {
							...
							if ($automaaginen == '') {
								if($laskurow['toim_osoite'] != $jtrow['toim_osoite'] and $laskurow['toim_postino'] != $jtrow['toim_postino'] and $laskurow['toim_postitp'] != $jtrow['toim_postitp']) {
									$toimitus_osoite = $jtrow['toim_osoite'] . ' ' . $jtrow['toim_postino'] . ' ' . $jtrow['toim_postino'];
									echo "<td><font class='error'>".t('Huom! J‰lkitoimitusrivill‰ on eri toimitusosoite.') . ' ' . $toimitus_osoite . "</font></td>";
								}
								echo "</tr>";
							}

<?php
	...
	$flock = fopen("/tmp/##tuote_export_cron", "w+");

	if (! @flock($flock, LOCK_EX | LOCK_NB)) {
		if (file_exists("/tmp/##tuote_export_cron") and mktime()-filemtime("/tmp/##tuote_export_cron") >= 5400) {
			echo "VIRHE: Tuote-export jumissa! Ota yhteys tekniseen tukeen!!!";

			// Onko nagios monitor asennettu?
			if (file_exists("/home/nagios/nagios-pupesoft.sh")) {
				file_put_contents("/home/nagios/nagios-pupesoft.log", "VIRHE: Tuote-export jumissa!", FILE_APPEND);
			}
		}
		exit;
	}

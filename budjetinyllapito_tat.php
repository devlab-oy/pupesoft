<?php
	...
	if (isset($tee) and $tee == "lataa_tiedosto") {
		...
		function tuotteenmyynti($tuoteno, $alkupvm) {
			...
			// niin funkkari palauttaa sen kuun myynnin arvon sek‰ kpl-m‰‰r‰n.
			global $kukarow, $toim;
							...
							if (mysql_num_rows($result) > 0) {
								...
								if ($budjrow['summa'] != $solu) {
									...
									elseif ($budj_kohtelu == "isoi" and $solu > 0.00) {
										...
										echo "<p class='error'>Virhe: Tˆrm‰ttiin virheeseen ja emme tallentaneet tietoa {$budj_taulu}-tauluun</p>";
										break 3;
									}
								...
								elseif ($tall_maara == 0 and $tall_index == 1 and $solu == 0.00 and $toim == "TUOTE" and $budj_kohtelu == "maara") {
									echo "<p class='error'>Virhe: Ei saa syˆtt‰‰ tyhj‰‰ {$budj_taulu}-tauluun</p>";
									break 3;
								}

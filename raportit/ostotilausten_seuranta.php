...
echo "<tr><th>",t("Päivämääräväli")," (",t("pp-kk-vvvv"),")</th>";
...
echo "<img src='".$palvelin2."pics/lullacons/bot-plain-red.png'/> = ",t("Saapumisen tuotteita ei ole viety varastoon tai saapumiseen ei ole liitetty yhtään riviä"),"<br/>";
echo "<img src='".$palvelin2."pics/lullacons/bot-plain-white.png'/> = ",t("Ostotilauksen tuotteita ei ole liitetty mihinkään saapumiseen"),"<br/>";
...
if (!is_numeric($ppa) or !is_numeric($kka) or !is_numeric($vva) or !is_numeric($ppl) or !is_numeric($kkl) or !is_numeric($vvl)) {
  echo "<br/><font class='error'>",t("Virheellinen päivämäärä"),"!</font>";
  $tee = '';
}
...
if ($tee == 'aja') {
  ...
  while ($toimittajarow = mysql_fetch_assoc($res)) {
    ...
    if (mysql_num_rows($tilrivi_res) > 0) {
      ...
      echo "<td class='spec'>",t("Yhteensä"),"</td>";

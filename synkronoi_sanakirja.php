...
if ($tee == "TEE" or $tee == "UPDATE") {
	...
	$file	 = fopen("http://www.devlab.fi/softa/referenssisanakirja.sql","r") or die (t("Tiedoston avaus ep‰onnistui")."!");
	$rivi    = fgets($file);
	...
	if (count($otsikot) > 1) {
		...
		if (isset($sync_otsikot["fi"])) {
			...
			while ($sanakirjarow = mysql_fetch_array($sanakirjaresult)) {
				...
				echo "<td>".$sanakirjarow["de"]."</td><td><font class='error'>".t("Puuttuu referenssist‰")."</font></td>";
				echo "</tr>";
			}

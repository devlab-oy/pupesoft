...
if (isset($task) and $task == 'suorita_eusiirto') {
  ...
  if (count($errors) > 0 or count($siirrettavat) == 0) {
    ...
    // pit‰‰kˆ perustaa uusi tulonumero
    $query = "SELECT *
              FROM lasku
    ...
    foreach ($siirrettavat as $rivitunnus => $tiedot) {
      ...
      // pit‰‰kˆ perustaa uusi tuote ja toimittaja
      $query = "SELECT *
                FROM tuote
                WHERE yhtio = '{$kukarow['yhtio']}'
      ...
      // pit‰‰kˆ kopioida uusi tilausrivi lis‰tietoineen
      $query = "SELECT *
                FROM tilausrivi
                WHERE yhtio = '{$kukarow['yhtio']}'
                AND tuoteno = '{$tuotenumero}'
...
if (isset($task) and $task == 'varaustaydennys') {
  $otsikko = t("T‰ydenn‰ tulon {$tulonumero} tiedot");
  $view = "tulotiedot";
}
...
//////////////////////////
// purkurasportin-lataus-n‰kym‰ alkaa
//////////////////////////

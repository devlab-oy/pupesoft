<?php
	...
	function teerivi($tuoteno, $valittu_toimittaja, $abc_rajaustapa) {
		...
		// Haetaan tuotteen tilauksessa oleva määrä
		...
		// Haetaan tuotteen varattu määrä (myynti)
		...
		// Haetaan valmistettavana oleva määrä
		...
		// Jos ollaan rajattu toimittaja käyttöliittymässä, haetaan sen tiedot eikä oletustoimittajaa
	...
	if (isset($tee) and $tee == "RAPORTOI") {
		...
		if (count($valmistettavat_tuotteet) > 0) {
			...
			foreach ($valmistettavat_tuotteet as $tuoterivi) {
				...
				echo t("Päiväkulutus")." = round(".t("Vuosikulutus")." / 360)<br>";
				...
				echo t("Määräennuste")." = (".t("Päiväkulutus")." * ".t("Toimitusaika").") + ".t("Myyntiennuste")."<br>";
				echo "{$tuoterivi["maaraennuste"]} = ({$tuoterivi["paivakulutus"]} * {$tuoterivi["toimitusaika"]}) + {$tuoterivi["myyntiennuste"]}<br><br>";
				echo t("Valmistussuositus")." = ceil(".t("Määräennuste")." - ".t("Reaalisaldo").")<br>";
	...
	if (!isset($tee) or $tee == "") {
		...
		echo "<tr>";
		echo "<th>".t("Alkupäivämäärä (kk-vvvv)")."</th>";
		echo "<td>";
		...
		echo "<tr>";
		echo "<th>".t("Loppupäivämäärä (kk-vvvv)")."</th>";
		echo "<td>";

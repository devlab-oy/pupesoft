...
elseif (in_array($valitsetoimitus, array("TARJOUS","PIKATILAUS","RIVISYOTTO","VALMISTAASIAKKAALLE","VALMISTAVARASTOON","SIIRTOLISTA","TYOMAARAYS", "TYOMAARAYS_ASENTAJA", "REKLAMAATIO","PROJEKTI"))) {
...
if ($tee == "tuotteetasiakashinnastoon") {
	...
	while ($tilausrivi = mysql_fetch_assoc($result)) {
		...
		echo "Lisättin tuote $tilausrivi[tuoteno] asiakkaan hinnastoon hinnalla ".sprintf("%.".$yhtiorow["hintapyoristys"]."f", $tilausrivi["hinta"] * (1 - $tilausrivi["ale"] / 100) * (1 - $laskurow["erikoisale"] / 100))." $laskurow[valkoodi]<br>";
	}
	$tee = "";
}
...
if ($tee == '') {
		...
		if ($laskurow["tunnusnippu"] > 0) {
			...
			if (mysql_num_rows($toimres) > 0) {
				...
				while($row = mysql_fetch_assoc($toimres)) {
					...
					echo "<option value ='$row[tunnus]' $sel $disabled>".t("$laskutyyppi")." $row[tilaus] ".t("$alatila")." $row[varasto]</option>";
				}
			}
	...
	if ((int) $kukarow["kesken"] != 0) {
		...
		if ($rivilaskuri > 0) {
			...
			if ($toim == "VALMISTAVARASTOON") {
				echo "<tr>$jarjlisa<td class='back' colspan='5' nowrap>";
			...
			if ($toim != "VALMISTAVARASTOON" and $toim != "SIIRTOLISTA") {
				...
				if ($kukarow['hinnat'] >= 0) $headerit .= "<th style='text-align:right;'>".t("Svh")."</th>";
				if ($kukarow['hinnat'] == 0) $headerit .= "<th style='text-align:right;'>".t("Ale%")."</th><th style='text-align:right;'>".t("Hinta")."</th>";
				if ($kukarow['hinnat'] >= 0) $headerit .= "<th style='text-align:right;'>".t("Rivihinta")."</th>";

				if ($kukarow['extranet'] == '' and ($kukarow["naytetaan_katteet_tilauksella"] == "Y" or ($kukarow["naytetaan_katteet_tilauksella"] == "" and $yhtiorow["naytetaan_katteet_tilauksella"] == "Y"))) {
					$headerit .= "<th style='text-align:right;'>".t("Kate")."</th>";
				}

				$headerit .= "	<th style='text-align:right;'>".t("Alv%")."</th>
						<td class='back'>&nbsp;</td>
						<td class='back'>&nbsp;</td>";
			}
			$headerit .= "</tr>";
			...
			if ($kukarow['hinnat'] != -1 and $toim != "SIIRTOTYOMAARAYS"  and $toim != "SIIRTOLISTA" and $toim != "VALMISTAVARASTOON") {
				...
				if ($laskurow["liitostunnus"] > 0 and $yhtiorow["tilauksen_jarjestys"] == "M" and in_array($toim, array("TARJOUS","PIKATILAUS","RIVISYOTTO","VALMISTAASIAKKAALLE","SIIRTOLISTA","TYOMAARAYS", "TYOMAARAYS_ASENTAJA", "REKLAMAATIO","PROJEKTI"))) {
					$ycspan++;
				}
				if ($kukarow['hinnat'] == 1) {
					$ycspan = $ycspan - 2;

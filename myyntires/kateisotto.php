...
function validoi_tapahtumapaiva($pp, $kk, $vv) {
  ...
  if (preg_match("/\d{4}-\d{2}-\d{2}/", $date) == false or strtotime($date) === false) {
    $errormsg = t("Päivämäärä on virheellinen");
    $return = false;
  }

  if (strtotime($date) > strtotime(date('Y-m-d'))) {
    $errormsg = t("Päivämäärä ei saa olla tulevaisuudessa");
    $return = false;
  }
...
function validoi_liitetiedosto($_FILES) {
  ...
  if (!empty($_FILES['userfile'])) {
    switch ($_FILES['userfile']['error']) {
      ...
      case 7:
      case 8:
        $errormsg .= t("Tallennus epäonnistui")."!<br/>";
        $return = false;
        break;
      case 0:
        //  OK tallennetaan
...
//Haetaan kaikki kassalippaat, joihin käyttäjällä on oikeus
//$kukarow['kassalipas'] pitää sisällään kassalippaan joihin oikeus. Jos tyhjä --> käyttäjällä oikeus kaikkiin kassalippaisiin

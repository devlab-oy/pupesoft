<?php
	...
	$query = "	SELECT suoritus.tunnus tunnus, 
				...
				tiliointi.korjattu=''";
	$result = mysql_query($query) or pupe_error($query);
	$suoritus = mysql_fetch_array ($result) or pupe_error('<br>Joku suoritukseen liittyvä tieto on kateissa (tämä on paha ongelma)'. $query);
	...
	if ($osasuoritus == 1) {
		...
		if (mysql_num_rows($result) != 1) {
			echo "Tiliöinti2 kateissa " . $suoritus["tunnus"];
			exit;
		}

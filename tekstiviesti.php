...
echo "<font class='head'>Tekstiviestien lähettäminen</font><hr>";
...
if ($tee == 'laheta') {
  ...
  foreach ($kotinums2 as $kotinum) {
    ...
    if ($ok == 1) {
      echo "<font class='error'>VIRHE: Tekstiviestin lähetys epäonnistui! $retval</font>";
      echo " ".$kotinum;
      echo "<br>";
    }

    if ($ok == 0) {
      echo "<font class='message'>Tekstiviesti lähetetty!</font>";
      echo " ".$kotinum;
      echo "<br>";
    }

    if ($ok == 2) {
      echo "<font class='message'>Samaa tekstiviestia ei lähetetty uudestaan samaan numeroon!</font>";
      echo " ".$kotinum;
      echo "<br>";
    }
...
function SendPupesoftSms($vastaanottaja, $viesti) {
  ...
  if (strlen($viesti) > 160) {
    echo "<font class='error'>VIRHE: Tekstiviestin maksimipituus on 160 merkkiä!</font><br><br>";
    return false;
  }
  ...
  if ($sms_palvelin == "" or $sms_user == "" or $sms_pass == "") {
    echo "<font class='error'>VIRHE: Tarkista tekstiviestipalvelimen määritykset asetustiedostossa!</font><br><br>";
    return false;
  }
...
function SendClickatellSms($vastaanottaja, $viesti) {
  ...
  if ($clickatell_api_id == "") {
    echo "<font class='error'>VIRHE: Tarkista asetukset! \$clickatell_api_id ei ole määritelty.</font><br/>";
    $continue = false;
  }
  if ($clickatell_username == "") {
    echo "<font class='error'>VIRHE: Tarkista asetukset! \$clickatell_username ei ole määritelty.</font><br/>";
    $continue = false;
  }
  if ($clickatell_password == "") {
    echo "<font class='error'>VIRHE: Tarkista asetukset! \$clickatell_password ei ole määritelty.</font><br/>";
    $continue = false;
  }
  if (!$continue) {
    echo "<br/><br/>";
...
function SendKannelSms($receiver_number, $message) {
  ...
  $continue = true;
  if ($kannel_host_url == "") {
    echo "<font class='error'>VIRHE: Tarkista asetukset! \$kannel_host_url ei ole määritelty.</font><br/>";
    $continue = false;
  }
  if ($kannel_username == "") {
    echo "<font class='error'>VIRHE: Tarkista asetukset! \$kannel_username ei ole määritelty.</font><br/>";
    $continue = false;
  }
  if ($kannel_password == "") {
    echo "<font class='error'>VIRHE: Tarkista asetukset! \$kannel_password ei ole määritelty.</font><br/>";
    $continue = false;
  }
  if (!$continue) {
    echo "<br/><br/>";

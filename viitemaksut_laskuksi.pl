...
# K‰yt‰ t‰t‰, jos et k‰yt‰ pupen myyntireskontraa.
# T‰ll‰ saa tehty‰ viiteaineistosta ihan vaan muistiotositteita
# Luetaan viitemaksuja...
# Tiliotes‰‰ntˆjen viites‰‰nnˆt ovat t‰‰ll‰ voimassa
# T‰m‰ on tarkoitettu ajettavaksi cronista
...
$dirri1 = "/var/pankki/";			# mist‰ dirikasta haetaan faileja
$dirri2 = "/var/pankki/kasitelty/";	# minne siirret‰‰n kun k‰sitelty
$dbhost = '';				# pupesoft hosti
$dbuser = '';				# pupesoft database k‰ytt‰j‰
$dbpass = '';				# pupesoft database salasana
$dbname = '';				# pupesoft databasen nimi
...
while ($file = readdir($hakemisto))
{
	...
	if (-f $nimi)
	{
		...
		my $sth_saanto=$dbh->prepare("SELECT tilino FROM tiliotesaanto WHERE yhtio = ? and locate(selite, ?) > 0 and pankkitili = 'x'");
		...
		my $sth_lukko=$dbh->prepare("LOCK TABLE tiliotedata WRITE, tiliointi WRITE, lasku WRITE, yriti READ, yhtio READ,   tiliotesaanto READ");
		...
		if (!@aineisto) {
			die "En saanut aineistonrowta luoduksi!";
		}
		...
		while (<F>) {
			...
			if ($type eq "0") {
				...
				$sth_tilioi->execute($yhtio, $entry_date, $last_id, $rahatili, $sum, $selite);
				$sth_tilioi->finish;
				#warn "luotiin tiliointi yhtiˆlle $yhtio";
		
				$sum = $sum * -1;
				
				#etsit‰‰n, jos kenties lˆytyisi tiliˆintis‰‰ntˆ!
				$sth_saanto->execute($yhtio, $payer_name);
				my (@saantotilinot)=$sth_saanto->fetchrow_array();
				$sth_saanto->finish;
				if (@saantotilinot) {
					$myynti = $saantotilinot[0];
					warn "Tiliˆintiin k‰ytettiin s‰‰ntˆ‰ $yhtio, $payer_name";
				}

				$sth_tilioi->execute($yhtio, $entry_date, $last_id, $myynti, $sum, $selite);
				$sth_tilioi->finish;
				$sth_last_id->execute();
				...
				#warn "luotiin tiliointi yhtiˆlle $yhtio";
				if ($otsikko ne "") {
					$sth_tiliotedata->execute($yhtio, $tamaaineisto, $account, $entry_date, $entry_date, 3, 0, $otsikko);
					$sth_tiliotedata->finish;
					$otsikko = "";
				}		
				$sth_tiliotedata->execute($yhtio, $tamaaineisto, $account, $entry_date, $entry_date, 3, $tilioi_id, $_);
				$sth_tiliotedata->finish;
				...
				$sth_tiliotedata->execute($yhtio, $tamaaineisto, $account, $entry_date, $entry_date, 3, 0, $_);
				$sth_tiliotedata->finish;
			} else {
...
#--TEST timestamp aineistoille ettei kirjoittaisi p‰‰lle

#		@time = localtime(time);
#		$nimi2 = "$file-" . ($time[5] + 1900) . "-" . $time[4] . "-" . $time[3] . "-";
# 		print("Done. $nimi\n");
# 		$cmd = "mv -f $nimi $dirri2" . "/" . $nimi2 . $nimi;
#-- TEST end

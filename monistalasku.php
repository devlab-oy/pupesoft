...
if ($toim == '' and $tee == 'MONISTA' and count($monistettavat) > 0) {
  ...
  foreach ($monistettavat as $lasku_x => $kumpi_x) {
    ...
    if ($kumpi_x == 'HYVITA') {
      // jos tilauksella on panttituotteita/sarjanumeroita pitää estää, että hyvitystä ei saa enää hyvittää (clearing=hyvitys)
      if ($chk_row['clearing'] == 'HYVITYS') {
        ...
        while ($chk_til_row = mysql_fetch_assoc($chk_til_res)) {
          if ($asiakas_panttitili_chk_row['panttitili'] == "K" and $chk_til_row['panttitili'] != '') {
            echo "<font class='error'>",t("Et voi hyvittää hyvityslaskua, jossa on panttitilillisiä tuotteita"),"! ({$lasku_x})</font><br>";
            $tee = "";
            break 2;
          }
          elseif ($chk_til_row["sarjanumeroseuranta"] != "") {
            echo "<font class='error'>",t("Et voi hyvittää hyvityslaskua, jossa on sarjanumerollisisa tuotteita"),"! ({$lasku_x})</font><br>";
            $tee = "";
            break 2;
          }
        }
        ...
        if (mysql_num_rows($clearing_chk_res) > 0) {
          ...
          while ($clearing_chk_row = mysql_fetch_assoc($clearing_chk_res)) {
            ...
            while ($chk_til_row = mysql_fetch_assoc($chk_til_res)) {
              if ($asiakas_panttitili_chk_row['panttitili'] == "K" and $chk_til_row['panttitili'] != '') {
                echo "<font class='error'>",t("Et voi hyvittää tilausta, jossa on panttitilillisiä tuotteita ja joka on jo hyvitetty"),"! ({$lasku_x})</font><br>";
                $tee = "";
                break 3;
              }
              elseif ($chk_til_row["sarjanumeroseuranta"] != "") {
                echo "<font class='error'>",t("Et voi hyvittää tilausta, jossa on sarjanumerollisia tuotteita ja joka on jo hyvitetty"),"! ({$lasku_x})</font><br>";
                $tee = "";
                break 3;
              }
            }
          ...
          if (mysql_num_rows($chk_til_res) > 0) {
            while ($chk_til_row = mysql_fetch_assoc($chk_til_res)) {
              ...
              if ($chk_til_row['kpl'] != $pantti_chk_row['kpl']) {
                echo "<font class='error'>",t("Hyvitettävän laskun pantit on jo käytetty"),"! ({$lasku_x})</font><br>";
                $tee = "";
                break 2;
              }
            }
    ...
    elseif ($asiakas_panttitili_chk_row['panttitili'] == "K" and $kumpi_x == 'MONISTA') {
      ...
      while ($chk_til_row = mysql_fetch_assoc($chk_til_res)) {
        ...
        if (mysql_num_rows($clearing_chk_res) == 0) {
          echo "<font class='error'>",t("Et voi monistaa tilausta, jossa on panttitilillisiä tuotteita ja se on hyvitetty, mutta hyvitystä ei ole laskutettu"),"! ({$lasku_x})</font><br>";
          $tee = "";
          break 2;
        }
      }
...
if ($tee == "ETSILASKU") {
  ...
  if (mysql_num_rows($result) > 0) {
    ...
    while ($row = mysql_fetch_array($result)) {
      ...
      if ($toim == '') {
        echo "<br />";
        echo "<input class='nayta_rivit' type='button' id='{$row['tilaus']}' value='",t("Näytä rivit"),"' />";
      }
      ...
      if ($toim == '') {
        ...
        echo "<input type='radio' id='rekla_{$row['tilaus']}' name='monistettavat[{$row['tilaus']}]' value='HYVITA' {$selhy} class='toiminnot_chk'>".t("Hyvitä")."<br>";
      ...
      if ($toim == '') {
        ...
        if ($toim == '') {
          ...
          echo "<input class='rekla_{$row['tilaus']}' {$display_none} type='checkbox' name='kaytetaanhyvityshintoja[{$row['tilaus']}]' value='on' {$sel}> <span {$display_none} class='rekla_{$row['tilaus']}'>",t("Käytetään reklamaatiolla hyvityshintoja"),"</span>";
        }

        echo "</{$ero}>";
      }

      echo "<{$ero} valign='top'><a href='?tunnus={$row['tilaus']}&tunnukset={$tunnukset}&asiakasid={$asiakasid}&otunnus={$otunnus}&laskunro={$laskunro}&ppa={$ppa}&kka={$kka}&vva={$vva}&ppl={$ppl}&kkl={$kkl}&vvl={$vvl}&tee=NAYTATILAUS&toim={$toim}'>".t("Näytä")."</a></{$ero}>";
...
if ($tee == 'MONISTA') {
  ...
  if (count($monistettavat) == 0) {
    echo "<font class='error'>Et valinnut yhtään laskua monistettavaksi/hyvitettäväksi</font><br>";
    $tee = "";
  }

<?php
	...
	$flock = fopen("/tmp/##hinnat_cron.lock", "w+");

	if (! @flock($flock, LOCK_EX | LOCK_NB)) {
		if (file_exists("/tmp/##hinnat_cron.lock") and mktime()-filemtime("/tmp/##hinnat_cron.lock") >= 5400) {
			echo "VIRHE: Hintojen päivitys jumissa! Ota yhteys tekniseen tukeen!!!";

			// Onko nagios monitor asennettu?
			if (file_exists("/home/nagios/nagios-pupesoft.sh")) {
				file_put_contents("/home/nagios/nagios-pupesoft.log", "VIRHE: Hintojen päivitys jumissa!", FILE_APPEND);
			}
		}
		exit;
	}

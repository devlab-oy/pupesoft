...
while (false !== ($file = readdir($handle))) {
  ...
  if ($is_tc === false and $is_txt === false) {
    pupesoft_log('smarten_tracking_code', "Tiedosto ei ole TC eikä TXT päätteinen, skipataan {$full_filepath}");
    continue;
  }
  ...
  while ($tietue = fgets($filehandle)) {
    ...
    if ($seurantakoodi == '') {
      pupesoft_log('smarten_tracking_code', "Seurantakoodi puuttuu riviltä");
      continue;
    }
    ...
    foreach ($tilausnumerot as $tilausnumero) {
      ...
      if ($tilausnumero == 0) {
        pupesoft_log('smarten_tracking_code', "Tilausnumero puuttuu riviltä");
        continue;
      }

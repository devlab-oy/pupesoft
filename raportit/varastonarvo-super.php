...
if (!$php_cli) {
  ...
  echo "<td>
      ...
      <option value='O' $sel1>".t("Näytetään tuotteet joilla on varauksia")."</option>
      <option value='E' $sel2>".t("Näytetään tuotteet joilla ei ole varauksia")."</option>
      </select>
      </td>";
  echo "</tr>";
  ...
  echo "<br><input type='checkbox' {$piilotetut_select} name='piilotetut_varastot' onclick='submit();' /> ".t('Näytä poistetut varastot');
...
if (isset($supertee) and $supertee == "RAPORTOI" or ($php_cli and $argv[0] == 'varastonarvo-super.php' and $argv[1] != '')) {
  ...
  if (isset($varatturajaus) and $varatturajaus != "") {
    ...
    // Näytetään vain varatut tuotteet
    if ($varatturajaus == "O") {
      $tuote_lisa .= " and tuote.tuoteno in ($varatut_tuotteet) ";
    }

    // Näytetään vain EI varatut tuotteet
    if ($varatturajaus == "E") {
  ...
  do {
    ...
    if ($row["sarjanumeroseuranta"] == "S" or $row["sarjanumeroseuranta"] == "U" or $row["sarjanumeroseuranta"] == "G") {
      ...
      while ($vararvorow = mysql_fetch_assoc($vararvores)) {
        ...
        // Jos meillä on eränumeroseuranta, niin otetaan erän koko sarjanumeroseurannan takaa. Sarjanumeroseurannassa aina yksi.
        ...
        // Jos meillä in-out arvoinen tuote, meidän pitää laskea varastonarvo ostohinnan mukaan

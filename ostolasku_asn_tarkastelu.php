			...
			while ($row = mysql_fetch_assoc($result)) {
				...
				elseif ($row['tilausrivi'] == '' and $row['tilausrivinpositio'] != '') {
					...
					if (mysql_num_rows($kpl_chk_res) == 0) {
						echo "<font color='orange'>Ostotilausrivi‰ ei lˆydy</font><br>";
					}
					...
					if ($kpl_row["otunnus"] != 0 and (strpos($kpl_row["otunnus"], $row['tilausnumero']) === FALSE)) {
						echo "<font color='scarlet'>Sanoman ostotilausnro ja <br>laskun ostotilausnro ei t‰sm‰‰<br> Tilaus: {$kpl_row["otunnus"]} <br>sanoma:{$row['tilausnumero']}</font><br>";
					}
					...
					if ($row['asn_sanomat'] == 'K') {
						...
						if (mysql_num_rows($chkres) == 0) {
							echo "<font color='error'>ASN-rivi‰ ei lˆydy tilausrivin tunnukselle $kpl_row[tunnus]</font><br>";
						}
					}

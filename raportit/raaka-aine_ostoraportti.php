...
if (isset($ehdotusnappi) and $ehdotusnappi != "") {
  ...
  if (mysql_num_rows($res) > 0) {
    ...
    while ($row = mysql_fetch_assoc($res)) {
      ...
      if ($tuoterivi['toimittajan_tunnus'] != $EDtoimittaja or $EDtoimittaja === false) {
        ...
        $toimittaja_header .= "<th>".t("Ostoerämäärä")."</th>";
        $toimittaja_header .= "</tr>";
        $toimittaja_header_piirretty = false;
      }
      ...
      // Jos tuotetta ei tarvitse ostaa, ei näytetä sitä ostoehdotuksessa
      ...
      // Pitää säilyttää table-headeria muuttujassa, sillä voi olla että toimittajalle ei tule yhtään tuoteriviä ehdotukseen (eikä haluta piirtää turhaa headeriä)
      ...
      echo "<tr><td>".t("Ostoerämäärä")."      </td><td>{$tuoterivi['ostoeramaara']}    </td></tr>";
      ...
      echo t("Ostettava määrä")." = round(".t("Ostosuositus")." / ".t("Pakkauskoko").") * ".t("Pakkauskoko")."<br>";
  ...
  if ($ostoehdotus_riveja == 0) {
    echo "<br><br>";
    echo "<font class='error'>Antamallasi rajauksella ei löydy yhtään tuotetta ehdotukseen.</font><br>";
    echo "<br>";
    $tee = "";
  }
}

<?php
	chdir('/var/www/html/pupesoft/tilauskasittely/');
	...
	// tiedoston ajo komentoriviltä
	$eraajo = 0;
	if (trim($argv[0]) != '' and trim($argv[1]) != '') {

		if ($argc == 0) die ("Tätä scriptiä voi ajaa vain komentoriviltä!");
		
		// otetaan includepath aina rootista
		ini_set("include_path", "..".PATH_SEPARATOR.ini_get("include_path").PATH_SEPARATOR.dirname(__FILE__).PATH_SEPARATOR."/usr/share/pear");
		...
		$query  = "	SELECT *
					FROM lasku
					WHERE tunnus  = '".trim($argv[1])."'";
		$vieres = mysql_query($query) or pupe_error($query);

		if (mysql_num_rows($vieres) == 1) {
			$laskurow = mysql_fetch_assoc($vieres);
		}
		else {
			echo "Tilausta ei löydy!\n";
			exit;
		}

		$yhtiorow = hae_yhtion_parametrit($laskurow['yhtio']);
		$kukarow["yhtio"] = $yhtiorow["yhtio"];
		$kukarow["eposti"] = trim($argv[2]);
	}
	else {

...
if ((int) $maksuehto != 0 and (int) $tunnus != 0) {
	...
	elseif ($laskupvmerror) {
		...
		echo "<font class='error'>".t("VIRHE: Tilikausi on päättynyt %s. Et voi merkitä laskua maksetuksi päivälle %s", "", $tilikausi['tilikausi_alku'], $tapahtumapaiva)."!</font>";
	}
}
...
function hae_lasku2($laskuno, $toim) {
	...
	elseif (!empty($tilikausi)) {
	 		echo "<font class='error'>".t("VIRHE: Tilikausi on päättynyt %s. Et voi muuttaa käteistä laskuksi %s", "", $tilikausi['tilikausi_alku'], $tapahtumapaiva)."!</font>";
	 		return FALSE;
	}

 ...
 ******************************************************/
  ...
  function draw_text($left, $bottom, $text, $parent, $attrib = array(), $iconvdone) {
    // K‰‰nnet‰‰n merkistˆ LATIN:iksi PDF-library ei hanskaa UTF-8:ia
    if ($iconvdone != "DONE") {
  ...
  function new_font($identifier) {
    // If the font's already been added, return the ID
    if (isset($this->builddata['fonts'])
      && is_array($this->builddata['fonts'])) {
      // If not, there are no fonts yet
      foreach ($this->builddata['fonts'] as $id => $name) {
        if ($name === $identifier) {
          return $id;
        }
      }
  ...
  function generate($clevel = 9) {
    ...
    /* First characters represent the version
         ...
         * a binary file
         */
    $os = "%PDF-1.3\x0a%\xe2\xe3\xcf\xd3\x0a";
    ...
    /* This part is more or less a hack to allow the importer to import
         * colorspaces.  The real solution is to teach the main library about
         * colorspaces so it can handle them
         */
    $xol = array();
    ...
    if (isset($xol) && count($xol) > 0) {
      $temp['ColorSpace'] = $this->_makedictionary($xol);
    }
    ...
    foreach ( $this->pdftolib as $pdfoid => $liboid ) {
      ...
      case 'raw':
        ...
        if (isset($this->objects[$liboid]['data'])) {
          $temp = $this->objects[$liboid];
          unset($temp['data']);
          unset($temp['type']);
          $os .= $this->_streamify($this->objects[$liboid]['data'],
            $temp);
          unset($temp);
        } else {
          $os .= $this->_makedictionary($this->objects[$liboid]);
        }
        $os .= " endobj";
        break;
  ...
  function add_raw_object($dict, $stream = '') {
    ...
    $this->objects[$o] = $dict;
    if (isset($dict['data'])) {
      $dict['data'] = '[Data stripped]';
    }
    $this->_debug_var(10, 'Added raw object', $dict);
    return $o;
  }
  ...
  function _streamify($data, $sarray = array()) {
    ...
    if (function_exists('gzcompress') && $this->builddata['compress']) {
      if ( !isset($sarray['Filter']) || strlen($sarray['Filter']) == 0 ) {
        ...
        if (strpos($sarray['Filter'], '/FlateDecode') === false)

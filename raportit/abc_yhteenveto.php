...
if ($asiakasanalyysi) {
  ...
  echo "<input type='radio' name='nayta_valinta' id='nayta_valinta' value='tuoteryhmittain' $nayta_select2>",t("Asiakasryhmittäin"),"<br/>";
}
else {
...
echo "<option value='TARK' $sel1>".t("Näytetään kaikki sarakkeet")."</option>";
if (!$asiakasanalyysi) echo "<option value='OSTONOHJ' $sel2>".t("Näytä simulointinäkymä")."</option>";
echo "</select></td>";
...
if ($tee == "YHTEENVETO") {
  ...
  if ($try == 'KAIKKI') {
    echo "<th nowrap><a href='$PHP_SELF?toim=$toim&tee=YHTEENVETO&nayta_valinta=$nayta_valinta&saapumispvm=$saapumispvm&lisatiedot=$lisatiedot$ulisa&order=".$orderurl."try&status=$status'>".t("Ryhmä")."</a><br>&nbsp;</th>";
  }
  ...
  if ($tuotemyyja == 'KAIKKI') {
    echo "<th nowrap><a href='$PHP_SELF?toim=$toim&tee=YHTEENVETO&nayta_valinta=$nayta_valinta&saapumispvm=$saapumispvm&lisatiedot=$lisatiedot$ulisa&order=".$orderurl."myyjanro&status=$status'>".t("Tuotemyyjä")."</a><br>&nbsp;</th>";
  }
  ...
  if ($lisatiedot == "TARK" or $lisatiedot == "OSTONOHJ") {
    ...
    echo "<th nowrap>$astusana<br>".t("määrä")."</th>";
  }
  ...
  if (!$asiakasanalyysi and $lisatiedot != "OSTONOHJ") {
    ...
    echo "<th nowrap>$myykusana<br>".t("määrä")."</th>";
  }

  if (!$asiakasanalyysi and ($lisatiedot == "TARK" or $lisatiedot == "OSTONOHJ")) {
    if ($lisatiedot == "TARK") {
      echo "<th nowrap>$myykusana".t("erä")."<br>".t("määrä")."</th>";
      echo "<th nowrap>$myykusana".t("erä")."<br>$yhtiorow[valkoodi]</th>";
      echo "<th nowrap>$myykusana<br>".t("rivejä")."</th>";
    }
    echo "<th nowrap>".t("Puute")."<br>".t("rivejä")."<br>".t("nyt")."</th>";

    if ($lisatiedot == "OSTONOHJ") {
      echo "<th nowrap>".t("Puute")."<br>".t("rivejä")."<br>".t("tavoite")."</th>";
    }
  }
  ...
  if ($lisatiedot == "OSTONOHJ") {
    ...
    echo "<th nowrap>".t("Ostoerä")."<br>".t("pv")."</th>";
  ...
  if (!$asiakasanalyysi and $lisatiedot == "TARK") {
    echo "<th nowrap>".t("Ostoerä")."<br>".t("määrä")."</th>";
  ...
  while ($row = mysql_fetch_assoc($res)) {
    ...
    if ($lisatiedot == "OSTONOHJ") {
      ...
      /*
      365 päivää /(( vuosimyynti yht kpl/(ostot yht kp/ ostokerrat ))

      Jos vuosimyyti on 500 kpl
      ostettu tuotetta 20 kertaa eli yhteensä 600 kpl

      kertaosto keskimäärin 30 kpl
      eli noin 22 päivää suosimyyntiin nähden.

      Vuosimyynti määrä = 500
      Vuosiostot määrä = 600
      Vuosiostot 20 kertaa

      Tästä alla olevalla kaavalla tulisi noin 22 (21,9) päivää riitäisi ostoerä.

      Keskimääräinen ostoerä on 30 kpl.
      Keskimäärin myydään 1,37 ( 500(365) kappaletta päivässä eli 30 kpl antaa
      näin tuon  noin 22 (21,9) päivää (30/1,37).
      Tässä tämä viimeinen vai tarkistuslaskenta.
      */
      ...
      if ($ryhmanimet[$row["luokka"]] == 'A-30' and 1==2) {
        ...
        echo "Vuosimyynti määrä = $row[kpl]<br>";
        echo "Vuosiostot määrä = $row[osto_kpl]<br>";
        echo "Vuosiostot $row[osto_kerrat] kertaa<br><br>";

        echo "Keskimääräinen ostoerä on ".round(($row["osto_kpl"] / $row["osto_kerrat"]), 0)." kpl. (".round($row["osto_kpl"],0)." / $row[osto_kerrat])<br>";
        echo "Keskimäärin myydään ".round(($row["kpl"] / 365), 2)." ($row[kpl] / 365) kappaletta päivässä<br>";
        echo "Vuodessa myydään riviä: $row[rivia]<br>";
        echo "eli ".round(($row["osto_kpl"] / $row["osto_kerrat"]), 0)." antaa näin tuon noin ".round(($row["osto_kpl"] / $row["osto_kerrat"]) / ($row["kpl"] / 365), 2)." päivää (".round(($row["osto_kpl"] / $row["osto_kerrat"]),2)." / ".round(($row["kpl"] / 365),2).")<br><br>";

...
if ((isset($tee) and $tee == 'TULOSTA') or (isset($otunnukset_array) and $otunnukset_array != '')) {
  ...
  if (isset($teekoontitarjous)) {
    echo "<font class='error'>",t("Valitse vähintään kaksi tarjousta"),"!</font><br /><br />";
  }

  echo "<form method='post'>";
  echo "<table>";
  echo "<tr><th>",t("Hae asiakas nimellä"),"</th><td><input type='text' name='asiakasnimi' value='{$asiakasnimi}' /></td><td class='back'>&nbsp;</td></tr>";
...
if ($submit and (trim($asiakasnimi) != '' or trim($asiakasytunnus) != '' or trim($asiakasnro) != '')) {
  ...
  if (mysql_num_rows($asiakasres) == 1) {
    ...
    if (mysql_num_rows($tarjousres) > 0) {
      ...
      echo "<font class='message'>",t("Tarjouksia löytyi")," ",mysql_num_rows($tarjousres)," ",t("kappaletta"),"! ",t("Vain ensimmäiset 100 tarjousta näytetään"),".</font><br /><br />";
      ...
      echo "<font class='message'>",t("Yhtään tarjousta ei löytynyt"),".</font><br />";
    }
  }
}

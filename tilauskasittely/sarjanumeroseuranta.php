		...
		while ($sarjarow = mysql_fetch_array($sarjaresiso)) {
			...
			if (($sarjarow["siirtorivitunnus"] > 0 and $tunnuskentta != 'siirtorivitunnus') or ($sarjarow["osto_perheid2"] > 0 and $sarjarow["osto_perheid2"] != $sarjarow["osto_rivitunnus"])) {
				...
				if ($siirow["tyyppi"] == "O") {
					...
					$fnlina1 .= "";
					$fnlina2 .= "<br><br>".t("Varattu lis‰varusteena").":<br>".$siirow["tuoteno"]." <a href='tilauskasittely/sarjanumeroseuranta.php?tuoteno_haku=$siirow[tuoteno]&sarjanumero_haku=".urlencode($siirow["sarjanumero"])."'>$siirow[sarjanumero]</a>";
				}

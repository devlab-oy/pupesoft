...
//t‰m‰ on naimisissa olevien osto ja myyntitilaus rivien saapumisten kautta poistamista varten
if($poista == 'Poista') {
	...
	if($tilausrivin_lisatiedot_row = mysql_fetch_assoc($result)) {
		//tilaukset on naitettu. poistetaan myynti jos ei ker‰tty, toimitettu tai laskutettu
		$query = "	SELECT *
					FROM lasku
					WHERE yhtio = '{$kukarow['yhtio']}'

<?php
	...
	if (isset($tee) and $tee == "lataa_tiedosto") {
		...
		if ($tee == "LASKUTA") {
			...
			// parametri, jolla voidaan estää tilauksen laskutus, jos tilauksen yhdeltäkin tuotteelta saldo menee miinukselle
					...
					if ($srow1["sarjanumeroseuranta"] != "") {
						...
						if ($tunken == "myyntirivitunnus") {
							...
							if (mysql_num_rows($sarres) > 0 and $srow2["ei_ostohintaa"] > 0) {
								...
								if ($silent == "" or $silent == "VIENTI") {
									$tulos_ulos_sarjanumerot .= t("$query Olet myymässä käytettyä venettä, jota ei ole vielä ostettu! Ei voida laskuttaa!").": $laskurow[tunnus] $srow1[tuoteno] $laskurow[nimi]!!!<br>\n";
								}
							}
						}
					}

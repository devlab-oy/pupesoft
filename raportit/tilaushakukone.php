			...
			for($i=0;$i<=$c; $i++) {
				...
				if(is_null($laskurow[$i]) and isset($ed[$i]) and $group_by != ""){
					if($i == 0) {
						...
						if(isset($workbook)) {
							$worksheet->write($excelrivi, $i, t("Kaikki yhteensä"), $format_loppusumma);
						}
					}
					else {
						echo "<td align='right' class='tumma'>".$laskurow[($i-1)]." ".t("yhteensä").":</td>";
				...
				elseif($ed[$i] != $laskurow[$i] or mysql_field_name($laskures, $i) == "Yhteensä") {

...
if (!empty($yhtiorow['editilaus_suoratoimitus'])) {
  ...
  if (mysql_num_rows($checkre) == 0) {
    ...
    $message = "Magento-tilaus oli jo kuitattu Magentoon ($_tilaukset). Ei tehdä sitä enää uudelleen tilaukselle $magento_api_laskutunnus ($magento_api_ord)";
    log_message($message);
  }
}
else {
...
//debuggauksen vuoksi, poistetaan tämä lopuksi
exit;

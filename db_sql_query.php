...
if (isset($tee)) {
  ...
  // tallennetaan aina myös kysely uudestaan jos sitä ajetaan (jos on oma rappari)
  if ($rtee == "AJA" and $kysely != '' and $kysely_kuka == $kukarow["kuka"]) {
    tallenna_muisti($kysely_mika, $ala_tallenna);
  }

  // jos kysely on valittuna mutta ei olla vielä ajamassa niin haetaan muuttujat
  ...
  if ($table!='') {
    ...
    echo "<table>";
    echo "<tr><td>".t("Ruksaa sisäänluvussa pakolliset kentät").":</td><td><input type='submit' name='ruks_pakolliset' value='".t("Ruksaa")."'></td></tr>";
    echo "</table><br><br>";

  ...
  elseif ($raportti != '') {
    ...
    if ($raportti == 'tapahtumia') {
      ...
      $query = "SELECT $select
                ...
                LEFT JOIN tuotteen_toimittajat ON (tuotteen_toimittajat.yhtio = tapahtuma.yhtio and tuotteen_toimittajat.tuoteno = tapahtuma.tuoteno)
    ...
    while ($tuoterow = mysql_fetch_assoc($saldoresult)) {
      ...
      if ($naytasaldo == 'H') {
        if ($rivipaikkahyllyssa != $rivivarastohyllyssa) {
          ...
          $excelrivit[$rivit]['hyllyssä'] =  $rivipaikkahyllyssa."(".$rivivarastohyllyssa.")";
        }
        else {
          $prn .= sprintf ('%-10.10s', $rivipaikkahyllyssa);
          $excelrivit[$rivit]['hyllyssä'] =  $rivipaikkahyllyssa;
        }
      }
      elseif ($naytasaldo == 'S') {
      ...
      $excelrivit[$rivit]['määrä'] = ' ';

      $prn .= sprintf ('%-9.9s',   t_avainsana("Y", "", "and avainsana.selite='$tuoterow[yksikko]'", "", "", "selite"));
      $excelrivit[$rivit]['yksikkö'] = t_avainsana("Y", "", "and avainsana.selite='$tuoterow[yksikko]'", "", "", "selite");

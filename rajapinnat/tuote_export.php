<?php
	...
	// jos meill‰ on lock-file ja se on alle 90 minuuttia vanha
	if (file_exists($lockfile)) {
		$locktime_calc = mktime() - filemtime($lockfile);

		if ($locktime_calc < $locktime) {
			exit("Tuote Export-p‰ivitys k‰ynniss‰, odota hetki!")."\n";
		}
		else {
			exit("VIRHE: Tuote Export-p‰ivitys jumissa! Ota yhteys tekniseen tukeen!!!")."\n";
		}
	}
	...
	while ($row = mysql_fetch_assoc($try_result)) {
		...
		// Ker‰t‰‰n myˆs pelk‰t tuotenumerot Magentoa varten
	...
	if (isset($verkkokauppatyyppi) and $verkkokauppatyyppi == "magento") {
		...
		if (count($kaikki_tuotteet) > 0) {
			...
			// HUOM, t‰h‰n passataan **KAIKKI** verkkokauppatuotteet, methodi katsoo ett‰ kaikki n‰m‰ on kaupassa, muut dellataan!

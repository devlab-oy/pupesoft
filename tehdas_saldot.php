...
if (PHP_SAPI != 'cli') {
  ...
  pupesoft_log('tehdas_saldot', "Aloitetaan tehtaan saldojen haku FTP:ll‰ osoitteesta {$ftpget_host['tehdas_saldot']}");
  ...
  if (empty($file)) {
    pupesoft_log('tehdas_saldot', "Yht‰‰n luettavaa tiedostoa ei lˆytynyt kansiosta {$ftpget_dest['tehdas_saldot']}");
    exit;
  }
}
  ...
  if ($error == 0) {
    ...
    if (strtolower($path_parts['extension']) == 'xls') {
      ...
      if ($file = fopen($_FILES['userfile']['tmp_name'], "r")) {
        ...
        if ($cli === true) {
          pupesoft_log('tehdas_saldot', "Tiedoston avaus ep‰onnistui: {$_FILES['userfile']['tmp_name']}");
        }
        else {
    ...
    if ($error == 0 and count($tuote) > 0) {
      ...
      if ($cli === true) {
        pupesoft_log('tehdas_saldot', "Tiedostosta ei luettu yht‰‰n tuotetta.");
      }
      else {
    ...
    if ($error == 0 and $tee == 'OK') {
      ...
      foreach ($tuote as $index => $tuoteno) {
        ...
        if ($cli === true) {
          ...
          if (mysql_num_rows($ttres) == 0) {
            pupesoft_log('tehdas_saldot', "Tuotteen toimittajaa ei lˆydy tuotteelle {$tuoteno}");
            continue;
          }
      ...
      if ($cli === true) {
        pupesoft_log('tehdas_saldot', sprintf(t('P‰ivitettiin %d tuotteen tehdas saldot', '', count($tuote))));
      }
      else {
        echo "<font class='message'>".sprintf(t('P‰ivitettiin %d tuotteen tehdas saldot', '', count($tuote))).".</font><br/>";
      }

<?php
	...
	if ($tee != "" and $tee != "MUUOTAOSTIKKOA") {
		...
		if ($tee == 'poista') {
			...
			//poistetaan myˆs myyntitilauksen tilausrivit
			$query = "	SELECT *
						FROM tilausrivi
			...
			while($tilausrivi_row = mysql_fetch_assoc($result)) {
				...
				if($tilausrivi_lisatiedot_row = mysql_fetch_assoc($tilausrivi_lisatiedot_result)) {
					//osto ja myynti on naimisissa kesken‰‰n.
					//myyntitilaus poistetaan vain jos sit‰ ei ole ker‰tty, toimitettu tai laskutettu
					$query = "	SELECT *
								FROM lasku
								WHERE yhtio = '{$kukarow['yhtio']}'
								AND tila = 'L'
					...
					if(mysql_num_rows($result) == 0) {
						...
						echo "<font class='message'>".t("Ostotilaukseen liitetyt myyntitilausrivit mit‰tˆitiin myˆs")."!<br><br></font>";
					}
				}
			}
		...
		if ($tee == 'PV') {
			...
			//jos myynti ja osto on naitettu kesken‰‰n niin asetetaan myyntiviri delete tilaan
			$query = "	SELECT *
						FROM tilausrivin_lisatiedot
						WHERE yhtio = '{$kukarow['yhtio']}'
		...
		if($tee == 'POISTA_RIVI') {
			...
			if($myyntitilausrivi_row = mysql_fetch_assoc($result)) {
				//tarkistetaan onko myyntitilausrivi‰ ker‰tty toimitettu tai laskutettu
				$query = "	SELECT *
							FROM tilausrivi
							WHERE yhtio = '{$kukarow['yhtio']}'
							AND tyyppi = 'L'
				...
				$result = pupe_query($query);
				if(mysql_num_rows($result) == 0) {
					//poistetaan myˆs myyntitilausrivi
					$query = "	UPDATE tilausrivi
								SET tyyppi = 'D'
								WHERE yhtio = '{$kukarow['yhtio']}'
								AND tunnus = '{$myyntitilausrivi_row['tilausrivitunnus']}'";
					...
					echo "<font class='error'>".t("Poistetaan rivi myˆs myyntitilaukselta")." {$myyntitilausrivi_row['vanha_otunnus']}</font><br/><br/>";
				}
				else {
					echo "<font class='error'>".t("Myyntitilaus rivi on keratty, toimitettu tai laskutettu")."</font><br/><br/>";
				}
			}
			$query = "	UPDATE tilausrivi
						SET tyyppi = 'D'
		...
		if ($tee == 'TI' and ((trim($tuoteno) != '' or is_array($tuoteno_array)) and ($kpl != '' or is_array($kpl_array))) and ($variaatio_tuoteno == "" or (is_array($kpl_array) and array_sum($kpl_array) != 0))) {
			...
			foreach ($tuoteno_array as $tuoteno) {
				...
				//tarkistetaan onko osto ja myyntitilaus rivit naitettu kesken‰‰n ja p‰ivitet‰‰n myˆs myyntitilausrivi. rivi lˆytyy jo kannasta tyypilla D
				$query = "	SELECT *
							FROM tilausrivin_lisatiedot
							WHERE yhtio = '{$kukarow['yhtio']}'
				...
				if($myyntitilausrivi_row = mysql_fetch_assoc($result)) {
					//tarkistetaan, ett‰ myyntitilausrivi‰ ei ole keratty, toimitettu, tai laskutettu
					$query = "	SELECT *
								FROM tilausrivi
								WHERE yhtio = '{$kukarow['yhtio']}'
								AND tyyppi = 'L'

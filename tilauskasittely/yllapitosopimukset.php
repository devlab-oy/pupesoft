...
// jos tullaan täältä itsestä niin tarvitaan paramertit
if (strpos($_SERVER['SCRIPT_NAME'], "yllapitosopimukset.php") !== FALSE) {
  require("../inc/parametrit.inc");

  echo "<font class='head'>".t("Ylläpitosopimukset")."</font><hr>";
  ...
  ob_start(); // ei echota mitäään jos kutsutaan muualta!
}
...
if ($tee == "laskuta" and count($laskutapvm) > 0) {
  ...
  foreach ($laskutapvm as $pointteri => $tapahtumapvm) {
    ...
    // Luodaan array jossa on sopimuksen tän vuoden kaikki laskutukset
    ...
    // Jos tämä tän vuoden eka sopparilasku, niin edellinen on viime vuoden vika
    ...
    // Jos tämä tän vuoden vika sopparilasku, niin seuraava on ens vuoden eka
    ...
    // Tämä ei ole tän vuoden eka eikä vika sopparilasku
    else {
      list($vv,$kk,$pp) = explode("-", $laskutuspaivat[$tama_laskutus-1]);
      $ed_alku = date("Y-m-d",mktime(0, 0, 0, $kk, $pp+1, $vv));
    ...
    if ($ok !== FALSE) {
      ...
      // päivitetään sopparipohjalle, että sitä on jo käyettty
      $query  = "UPDATE lasku
                 SET alatila = 'X'
      ...
      // tyyppi takasin L, merkataan rivit kerätyks ja toimitetuks
      ...
      if ($jatakesken != "JOO") {
        ...
        $laskuta_message .= ", ".t("lasku jätettiin avoimeksi").".</font><br>";
      }
    }
  }
...
if (mysql_num_rows($result) > 0) {
  ...
  while ($row = mysql_fetch_assoc($result)) {
    ...
    // katotaan montakertaa tää on laskutettu tai laskuttamatta
    $laskutettu = "";
    $laskutettu_vika = "";
    $laskuttamatta = "";
    ...
    // näyttään vaan tähän päivään asti
  ...
  echo t("Ei ylläpitosopimuksia").".";
}

// jos tullaan täältä itsestä niin näytetään footer
if (strpos($_SERVER['SCRIPT_NAME'], "yllapitosopimukset.php") !== FALSE) {
  require ("inc/footer.inc");
}
else {
  ob_end_clean(); // ei echota mitäään jos kutsutaan muualta!
}

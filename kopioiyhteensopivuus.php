  ...
  if ($errori == '') {
    ...
    for ($z = 0; $z < 2; $z++) {
      ...
      for ($i=1; $i < mysql_num_fields($tresult) -1; $i++) {
        ...
        elseif (mysql_field_name($tresult, $i) == "muuttaja") {
          $query .= mysql_field_name($tresult, $i)."='{$kukarow['kuka']}',";
        }
        elseif (mysql_field_name($tresult, $i) == "luontiaika") {
          $query .= mysql_field_name($tresult, $i)."=now(),";
        }
        elseif (mysql_field_name($tresult, $i) == "laatija") {
          $query .= mysql_field_name($tresult, $i)."='{$kukarow['kuka']}',";
        }
        elseif (mysql_field_name($tresult, $i) == "tunnus" and $z == 0) {
          //jätetään pois
        }
        else {
          if ($z == 0) {
    ...
    while ($row = mysql_fetch_array($result)) {
      ...
      for ($i = 0; $i < mysql_num_fields($result) - 1; $i++) {
        ...
        elseif (mysql_field_name($result, $i) == "muuttaja") {
          $yhteensopivuus_query .= mysql_field_name($result, $i)."='{$kukarow['kuka']}',";
        }
        elseif (mysql_field_name($result, $i) == "luontiaika") {
          $yhteensopivuus_query .= mysql_field_name($result, $i)."='now()',";
        }
        elseif (mysql_field_name($result, $i) == "laatija") {
          $yhteensopivuus_query .= mysql_field_name($result, $i)."='{$kukarow['kuka']}',";
        }
        elseif (mysql_field_name($result, $i) == "atunnus") {
...
if ($tee == '') {
  ...
  echo "<table><form method = 'post'><tr>";

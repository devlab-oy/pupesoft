...
// Mikä on oikea ostovelkatili
		$query = "SELECT tilino
				...
				abs(summa + $laskurow[vietysumma]) <= 0.02 and tilino in ('$yhtiorow[ostovelat]', '$yhtiorow[konserniostovelat]') and korjattu=''";
...
// Valuutta-ero
		if ($laskurow['vienti_kurssi'] != $kurssi) {
			...
			if ($totvesumma != $vesumma) {
				...
				$query = "UPDATE tiliointi SET summa = summa - $totvesumma + $vesumma
						WHERE tunnus = '$isa' and yhtio='$kukarow[yhtio]'";
				$xresult = mysql_query($query) or pupe_error($query);
			}
		}
...
// Näytetään tiliöinnit
		...
		echo "<form action = '$PHP_SELF' method='post'>
			...
			<input type='hidden' name='tiliote' value='$tiliote'>
				...
				<td><input type='Submit' value='".t("Suorita lisää")."'></td></form>";
		echo "<form action = 'muutosite.php' method='post'>
				<input type='hidden' name='tee' value='E'>
				<input type='hidden' name='tunnus' value='$tunnus'>
			<td><input type='Submit' value='".t("Korjaa tiliöinti")."'></td></tr></form></table>";
	}

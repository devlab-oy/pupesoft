...
if ($toiminto == "poista") {
	...
	if ($eisaapoistaa == 0) {
		...
		echo "<font class='error'>".t("VIRHE: Keikkaan on jo liitetty laskuja tai kohdistettu rivej‰, sit‰ ei voi poistaa")."!<br>";
		// formissa on tullut myˆs $ytunnus, joten n‰in p‰‰st‰‰n takaisin selaukseen
		$toiminto = "";
	}
}
...
if ($ytunnus == "" and $keikka != "") {
	$keikka = (int) $keikka;
	$query = "	SELECT ytunnus, liitostunnus, max(laskunro)
	...
	if ($keikkahaku_row['laskunro'] == '' and $keikkahaku_row['ytunnus'] == '') {
		...
		$keikkahaku_row = mysql_fetch_assoc($ytunnus_res);
	}

...
if ($tee == 'LISTAAOLETUKSET') {
	...
	if ($num_rows > 0) {
		...
		if ($num_rows < 500) {
			...
			echo t("Virheellisiä oletuspaikkoja on %s. Niitä ei näytetä ruudulla.", "", $num_rows)."<br>";
		}
		...
		echo t("Ei löytynyt yhtään virheellistä oletuspaikkaa")."!<br><br>";
		$tee = "";
	}
}
...
if ($tee == 'LISTAARIKKINAISET') {
	...
	if (mysql_num_rows($result) > 0) {
		...
		echo t("Ei löytynyt yhtään virheellistä varastopaikkaa")."!<br><br>";
		$tee = "";
	}
}
...
if ($tee == "LISTAAVIRHEELLISETRIVIT") {
	...
	if ($laskuri > 0) {
		echo "<br>".t("Yhteensä %s tilausriviä.", "", $laskuri)."<br>";
...
if ($tee == "LISTAATAPAHTUMATILMANPAIKKAA") {
	...
	if ($laskuri > 0) {
		...
		echo t("Kaikki tapahtumien tuotepaika löytyvät")."!<br><br>";
		$tee = "";
	}
	echo "</form>";
}
...
if ($tee == "") {
	...
	echo "<td><input type='checkbox' name='ei_nayteta_riveja'> ",t("Älä listaa tapahtumia ruudulla"),"</td>";
	echo "<td class='back'><input type='submit' value='".t("Hae")."'></td></tr>";
	echo "</form>";

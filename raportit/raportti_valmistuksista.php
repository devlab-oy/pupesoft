...
echo "<option value=''>".t('Näytetään tuotteittain')."</option>";
$sel = '';
if ($esitysmuoto == 'Y') {
  $sel = 'SELECTED';
}
echo "<option value='Y' {$sel}>".t('Näytetään vain yhteensärivit')."</option>";
echo "</select>";
echo "</td>";
echo "</tr>";
...
if ($tee == "ajaraportti" and isset($submit_nappi)) {
  ...
  if (mysql_num_rows($result) > 0) {
    ...
    if (isset($workbook)) {
      ...
      $excelsarake++;
      $worksheet->write($excelrivi, $excelsarake, t("Keräysajankohta"), $format_bold);
      $excelsarake++;
      $worksheet->write($excelrivi, $excelsarake, t("Valmistusajankohta"), $format_bold);
      $excelsarake++;
      $worksheet->write($excelrivi, $excelsarake, t("Toteutunut valmistuspäivä"), $format_bold);
      $excelsarake++;
    }
    ...
    if ($esitysmuoto == '') {
      ...
      echo "<th>".t("Valmistuksen tila")."</th>";
      echo "<th>".t("Keräysajankohta")."</th>";
      echo "<th>".t("Valmistusajankohta")."</th>";
      echo "<th>".t("Toteutunut Valmistuspäivä")."</th>";
      echo "</tr>";
    ...
    while ($rivit = mysql_fetch_assoc($result)) {
      ...
      if($linja == "") $linja = t("Ei määritelty");
    ...
    if ($esitysmuoto == '') {
      ...
      echo "<tfoot>";
      echo "<tr>";
      echo "<th colspan='6'>".t("Näkymä yhteensä").":</th>";
    ...
    foreach ($yhteenveto_valmistettu as $valmistuslinja => $value) {
      ...
      if ($linja_yht == "") $linja_yht = t("Ei Määritelty");
    ...
    echo "<br><font class='error'>".t("Yhtään tuotetta ei löytynyt")."!</font>";
  }
}

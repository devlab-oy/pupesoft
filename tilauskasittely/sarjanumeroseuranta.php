...
if ($from != '' and $rivitunnus != "") {
  ...
  elseif ($rivirow["varattu"] < 0 and ($from == "riviosto" or $from == "kohdista")) {
    // tässä muutetaan ostorivitunnus myyntirivitunnukseksi jos $rivirow["varattu"] eli kappalemäärä on negatiivinen
    $tunnuskentta     = "myyntirivitunnus";
    $rivirow["varattu"] = abs($rivirow["varattu"]);
    $ostonhyvitysrivi   = "ON";
  }
  ...
  if (isset($PAIVITA)) {
    ...
    if (mysql_num_rows($muutares) == 1) {
      ...
      if ($muutarow["myyntirivitunnus"] > 0) {
        if ($muutarow["sarjanumeroseuranta"] == "E" or $muutarow["sarjanumeroseuranta"] == "F" or $muutarow["sarjanumeroseuranta"] == "G") {
          echo "<br><br><font class='error'>".t("HUOM: Eränumero on liitetty tilaukseen! Tilauksen tiedot muuttuvat jos muokkaat eränumeroa.")."</font>";
        }
        else {
      ...
      if ($muutarow["sarjanumeroseuranta"] == "E" or $muutarow["sarjanumeroseuranta"] == "F" or $muutarow["sarjanumeroseuranta"] == "G") {
        if ($muutarow["era_kpl"] >= 0 and $muutarow["myyntirivitunnus"] == 0 and ($muutarow["ostorivitunnus"] == 0 or $from == "kohdista")) {
          ...
          echo "<tr><th>".t("Erän suuruus")."</th><td>$muutarow[era_kpl]</td></tr>";
...
if ($toiminto == 'LISAA' and trim($sarjanumero) != '') {
  ...
  if ($rivirow["sarjanumeroseuranta"] == "S" or $rivirow["sarjanumeroseuranta"] == "T" or $rivirow["sarjanumeroseuranta"] == "U" or $rivirow["sarjanumeroseuranta"] == "V") {
    ...
    if ((float) $era_kpl > $rivirow["varattu"]) {
      $insok = "EI";
      echo "<font class='error'>".t("Erän koko on liian suuri")." $rivirow[varattu]/$era_kpl.</font><br><br>";
    }

    if ($from == "KERAA" and (float) $era_kpl != $rivirow["varattu"]) {
      $insok = "EI";
      echo "<font class='error'>".t("Erän koko on oltava sama kuin rivin määrä")." $rivirow[varattu]/$era_kpl.</font><br><br>";
    }
...
if ($from != '' and $rivitunnus != "" and $formista == "kylla") {
  ...
  if (count($sarjataan) > 0 and ($rivirow["sarjanumeroseuranta"] == "E" or $rivirow["sarjanumeroseuranta"] == "F" or $rivirow["sarjanumeroseuranta"] == "G")) {
    ...
    if (($from == "KERAA" or $from == "valmistus" or $from == "VALMISTAASIAKKAALLE" or $from == "VALMISTAVARASTOON") and $rivirow["varattu"] != $sarrow["kpl"]) {
      echo "<font class='error'>".t('Riviin voi liittää vain')." $rivirow[varattu] ".t_avainsana("Y", "", "and avainsana.selite='$rivirow[yksikko]'", "", "", "selite").".</font><br><br>";

      $lisaysok = "";
    }
    elseif ($rivirow["varattu"] < $sarrow["kpl"]) {
      echo "<font class='error'>".t('Riviin voi liittää enintään')." $rivirow[varattu] ".t_avainsana("Y", "", "and avainsana.selite='$rivirow[yksikko]'", "", "", "selite").".</font><br><br>";

      $lisaysok = "";
    }
  }
  // Tutkitaan koitetaanko salaa liittää enempi kuin $rivirow["varattu"]
  ...
  // Tutkitaan ettei liitetä sekä uusia että vanhoja sarjanumeroita samaan riviin
  if (count($sarjataan) > 0) {
    ...
    if (mysql_num_rows($sarres) > 1) {
      echo "<font class='error'>".t('Riviin ei voi liittää sekä käytettyjä että uusia sarjanumeroita')."</font><br><br>";
  ...
  elseif (($rivirow["varattu"] < count($sarjataan) and ($rivirow["sarjanumeroseuranta"] == "S" or $rivirow["sarjanumeroseuranta"] == "T" or $rivirow["sarjanumeroseuranta"] == "U" or $rivirow["sarjanumeroseuranta"] == "V")) or (($rivirow["sarjanumeroseuranta"] == "E" or $rivirow["sarjanumeroseuranta"] == "F" or $rivirow["sarjanumeroseuranta"] == "G") and ($rivirow["varattu"] == 0 or count($sarjataan) == 0))) {
    echo "<font class='error'>".sprintf(t('Riviin voi liittää enintään %s sarjanumeroa'), abs($rivirow["varattu"])).". ".$rivirow["varattu"]." ".count($sarjataan)."</font><br><br>";
  }
...
if ($rivirow["tuoteno"] != '') {
  ...
  echo "<tr><th>".t("Määrä")."</th><td>$rivirow[varattu] ".t_avainsana("Y", "", "and avainsana.selite='$rivirow[yksikko]'", "", "", "selite")."</td></tr>";
  ...
  while ($sarjarow = mysql_fetch_assoc($sarjaresiso)) {
    ...
    if ($rivirow["sarjanumeroseuranta"] == "E" or $rivirow["sarjanumeroseuranta"] == "F" or $rivirow["sarjanumeroseuranta"] == "G") {
      ...
      if ($sarjarow["era_kpl"] < 0) {
        echo "<br>".t("Erässä").": ".abs($sarjarow["era_kpl"])." ".t_avainsana("Y", "", "and avainsana.selite='$rivirow[yksikko]'", "", "", "selite")."<br><font class='error'>".t("Erä on jo myyty")."!</font>";
      }
      else {
        echo "<br>".t("Erässä").": $sarjarow[era_kpl] ".t_avainsana("Y", "", "and avainsana.selite='$rivirow[yksikko]'", "", "", "selite");
      }
    }
...
if ($rivirow["tuoteno"] != '') {
  ...
  if ($rivirow["tuoteno"] != '' and ($rivirow["sarjanumeroseuranta"] == "E" or $rivirow["sarjanumeroseuranta"] == "F" or $rivirow["sarjanumeroseuranta"] == "G")) {
    ...
    echo "<br><table>";
    echo "<tr><th colspan='2'>".t("Lisää uusi eränumero")."</th></tr>";
    echo "<tr><th>".t("Eränumero")."</th><td><input type='text' size='30' name='sarjanumero' value='$sarjanumero'></td><td class='back'><a href='#' onclick='document.sarjaformi.sarjanumero.value=\"$nxt\";'>".t("Seuraava erä")."</a></td></tr>";

    echo "<tr><th>".t("Erän suuruus")."</th><td><input type='text' size='30' name='era_kpl' value='$era_kpl'></td></tr>";

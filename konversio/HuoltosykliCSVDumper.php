		...
		foreach ($rivi as $key => $value) {
			...
			if ($key == 'laite') {
				...
				if ($attrs == 1) {
					...
					$this->errors[$index][] = t('Huoltosyklin laitteelle')." <b>{$rivi[$key]}</b> ".t('lˆytyi 0 laitteen koko tai tyyppi‰');
					$loytyyko_laite_tuote = $this->loytyyko_tuote($rivi[$key]);
					if (!$loytyyko_laite_tuote) {
						$this->errors[$index][] = t('Laitetta')." <b>{$rivi[$key]}</b> ".t('ei lˆytynyt');
					}
					$valid = false;
				}
				else {
				...
				if (!$valid_temp) {
					$this->errors[$index][] = t('Toimenpide tuotetta')." <b>{$rivi[$key]}</b> ".t('ei lˆytynyt');
					if ($valid) {
						$valid = false;
					}
				}

      ...
      $(function() {
        ...
        $('#siirtotable').on('click', '#yhdistabutton', function(e) {
          ...
          if ($('.siirtolistan_vastaanotto:checked').length > 1) {
            ...
            alert('", t("Valitse v�hint��n 2 siirtolistaa"), "');
          }
        });
      });
    </script>";
...
if ($tee == 'paikat') {
  ...
  //k�yd��n kaikki rivit l�pi ja tarkastetaan varastopaikka ja perustetaan uusia jos on tarvis
  foreach ($tunnus as $tun) {
    ...
    if (mysql_num_rows($result) != 1) {
      lisaa_virhe("<font class='error'>", "VIRHE: Rivi� ei l�ydy tai se on jo siirretty uudelle paikalle", "</font><br>");
    }
    elseif ($tilausrivirow["ei_saldoa"] == "") {
      ...
      if (mysql_num_rows($result) == 0) {
        ...
        if (mysql_num_rows($vares) == 1) {
          ...
          if ($t1[$tun] != '' and $t2[$tun] != '' and $t3[$tun] != '' and $t4[$tun] != '') {
            ...
            lisaa_virhe("<font class='error'>", t("VIRHE: Tuotenumerolle")." $tilausrivirow[tuoteno]. ".t("ei voitu perustaa tyhj�� varastopaikkaa"), "</font><br>");
            $virheita++;
          }
        }
        else {
          lisaa_virhe("<font class='error'>", t("VIRHE: Sy�tt�m�si varastopaikka ei kuulu kohdevaraston alueeseen"), "</font><br>");
        ...
        if ($paikkarow["inventointilistatunnus"] !== null) {
          lisaa_virhe("<font class='error'>", "paikkarow[hyllyalue]-$paikkarow[hyllynro]-$paikkarow[hyllyvali]-$paikkarow[hyllytaso] ".t("VIRHE: Kohdepaikalla on inventointi kesken, ei voida jatkaa"), "</font><br>");
          $virheita++;
        }
      ...
      if ($virheita === 0) {
        ...
        // P�ivitet��n sy�tetyt paikat tilausrivin_lis�tietoihin
    ...
    if ($prow["tunnus"] !== null) {
      lisaa_virhe("<font class='error'>", "$tilausrivirow[hyllyalue]-$tilausrivirow[hyllynro]-$tilausrivirow[hyllyvali]-$tilausrivirow[hyllytaso] ".t("VIRHE: L�hdepaikalla on inventointi kesken, ei voida jatkaa"), "</font><br>");
      $virheita++;
    }
  }
...
if ($tee == 'valmis') {
  ...
  // Tehd��n lukko, jotta vain yksi ajo kerrallaan
  $lock_params = array(
    "return"   => true,
  );

  if (!pupesoft_flock($lock_params)) {
    lisaa_virhe("<font class='error'>", t("Ruuhkaa varastosiirtojen vastaanotossa, yrit� my�hemmin uudelleen!"), "</font><br>");
    exit();
  }
  ...
  foreach ($tunnus as $tun) {
    ...
    if (mysql_num_rows($result) != 1) {
      lisaa_virhe("<font class='error'>", t("VIRHE: Rivi� ei l�ydy tai se on jo siirretty uudelle paikalle"), "</font><br>");
    }
    else {
      $tilausrivirow = mysql_fetch_assoc($result);
      ...
      if ($asaldo != 0 and $tilausrivirow["ei_saldoa"] == "") {
        ...
        if (mysql_num_rows($presult) == 0) {
          lisaa_virhe("<font class='error'>", t("VIRHE: Antavaa varastopaikkaa ei l�ydy")." $tuoteno!", "</font>");
          if ($echotaanko) {
            exit;
          }
          else {
            // jos antavaa paikka ei l�ydy, niin lis�t��n se jos ajetan komentorivilt�
            lisaa_virhe("<font class='error'>", t("VIRHE: Antavaa varastopaikkaa ei l�ydy")." $tuoteno", "</font>");
            $lisatty_paikka = lisaa_tuotepaikka($tilausrivirow["tuoteno"], $tilausrivirow["hyllyalue"], $tilausrivirow["hyllynro"], $tilausrivirow["hyllyvali"], $tilausrivirow["hyllytaso"], 'Varastosiirron vastaanotossa, koska l�hdepaikka oli kateissa', '', 0, 0, 0);
        ...
        if (mysql_num_rows($presult) != 1) {
          lisaa_virhe("<font class='error'>", t("VIRHE: Vastaanottavaa varastopaikkaa ei l�ydy")." $tuoteno!", "</font>");
        }
        else {
          $prow = mysql_fetch_assoc($presult);
...
if (empty($id) and $echotaanko) {
  ...
  while ($tilrow = mysql_fetch_assoc($tilre)) {
    ...
    if ($_suljettu_lahto) {
      //Ei n�ytet� siirtolistoja, joihin on liitetty aukioleva l�ht�
      continue;
    }
...
if (!empty($id) and $echotaanko) {
  ...
  while ($rivirow = mysql_fetch_assoc($result)) {
    ...
    echo "<td align='center'><input type='checkbox' id='vastaanotetaan[$rivirow[tunnus]]' name='vastaanotetaan[$rivirow[tunnus]]' checked>";
    ...
    echo "<td><input type='text' id='vastaanotettu_maara[$rivirow[tunnus]]' name='vastaanotettu_maara[$rivirow[tunnus]]' value='$rivirow[varattu]' maxlength='10' size='10'></td>";
      ...
      if ($rivirow["ei_saldoa"] != "") {
        ...
        if (array_sum($counts) > 1) {
          ...
          if (array_sum($counts) == 1) {
            ...
            lisaa_virhe("<font class='error'>", t("Ei varastopaikkaa"), "</font></td>");
          }
        }
  ...
  if ($toim != "MYYNTITILI") {
    ...
    echo t("J�rjest� tuotenumeron mukaan")."<br><input type='checkbox' name='jarjestys'></td>";
    echo "</table>";
  }
  echo "<br>";

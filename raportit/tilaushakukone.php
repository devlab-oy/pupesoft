...
if($tee == "") {
	...
	if($toim == "TARJOUSHAKUKONE") {
		...
		if(count($group)>0) {
			...
			$q = "lasku.tunnus tarjous, laskun_lisatiedot.seuranta, concat_ws('<br>',versio.nimi, versio.nimitark) asiakas, asiakkaan_kohde.kohde, lasku.laatija, $summa Yhteensä, if(versio.alatila IN ('', 'A'), DATEDIFF(versio.luontiaika, date_sub(now(), INTERVAL laskun_lisatiedot.tarjouksen_voimaika day)), '') pva, lasku.tila, lasku.alatila,";
			$c = 7;			
		}

  ...
  foreach ($tilausrivit as $rivitunnus => $data) {
    ...
    if ($kpl < $tilausrivirow['varattu']) {
      ...
      if ($uusi_id != 0) {
        ...
        pupesoft_log('smarten_inbound_delivery_confirmation', "Tilausrivi {$rivitunnus} splitattiin. Uusi käytetty tunnus on {$uusi_id}. Saavutettiin {$kpl}, saavuttamatta {$erotus}.");

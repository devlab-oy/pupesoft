...
if ($subnappi != '') {
	...
	while ($row = mysql_fetch_array($result)) {
		...
		if ($saapunut < $alaraja) {
			...
			if ($kierto < $raja) {
				echo "$row[osasto]\t$row[try]\t".str_replace(".",",",$myyrow['kpl'])."\t".str_replace(".",",",$saldo)."\t".str_replace(".",",",$kierto)."\t$row[tahtituote]\t$taprow[min]\t$taprow[max]\t".str_replace(".",",",$row['myyntihinta'])."\t".str_replace(".",",",$row['kehahin'])."\t$row[tuoteno]\t$row[nimitys]\t$row[toimittaja]\n";
			}

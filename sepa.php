...
//				$Nm = $CdtrAgtAcct->addChild('Nm', '');
			...
			if (trim($laskurow['pankki_haltija']) != '') {
				$Nm = $Cdtr->addChild('Nm', sprintf("%-1.70s", htmlentities($laskurow['pankki_haltija'])));												// Name, Pakollinen kentt‰ 1-70
			}
			else {
				$Nm = $Cdtr->addChild('Nm', sprintf("%-1.70s", htmlentities($laskurow['nimi'])));														// Name, Pakollinen kentt‰ 1-70
			}

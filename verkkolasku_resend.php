  ...
  foreach ($mave_laskut as $yhtio => $laskut) {
    ...
    foreach ($laskut as $laskunro => $lasku) {
      ...
      if ($status != 'OK: INVOICE CREATED SUCCESSFULLY') {
        ...
        $maventaerrorreport .= "Teknisemp‰‰ tietoa virheest‰:\n";
        $maventaerrorreport .= "Status: $status\n\n";

        // Printataan virhe myˆs Pupen omaan errorlogiin
        error_log(print_r("Virhe laskun {$laskunro} l‰hetyksess‰ Maventaan, status: {$status}", true));

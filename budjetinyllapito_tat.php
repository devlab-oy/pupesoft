<?php
	...
	// Mik‰li teet muutoksia niin muuta myˆs riveille alkaen 1170 !!!
	...
	// T‰m‰ funkkari palauttaa syˆtetyn tuotteen sen kuukauden myynnin arvon ja kpl-m‰‰r‰n
	...
	if (isset($luvut) and count($luvut) > 0 and $submit_button != '') {
		...
		// Seuraavassa k‰sitell‰‰n tilikautta josta erotuksesta saadaan jakaja.
		// muuttujaa '$jakaja' k‰ytet‰‰n sek‰ "update" ett‰ "insert" puolella.
		// $jakaja on kuukausien lukum‰‰r‰

		if ((($budj_kohtelu == "euro" or $budj_kohtelu == "maara") and $budj_taso == "summataso") or $summabudjetti == "on") {
			// jos k‰sinsyˆtetty aikav‰li on tyhj‰‰, niin haetaan dropdownin valinta
			if ($alkukk == "" and $loppukk == "" and $tkausi !="") {
				...
				// jos ollaan syˆtetty k‰sin kausi
		...
		if ($poikkeus == "totta") {
			...
			elseif ($poikkeus_haku == "kummatkin") {
				...
				// mik‰li 2 edellist‰ haaraa ep‰onnistuu, niin keskeytet‰‰n k‰sittely
				// syy t‰h‰n haaraan p‰‰tymiselle on riveill‰ noin 1173, 1178, 1183
				// mik‰li sinne kirjoitetaan enemm‰n tai muutetaan nimityksi‰ niin ne pit‰‰ korjata t‰h‰n kohtaan myˆs !!
				echo "<p class='error'>".t("Virhe: Budjetinluonnissa on tapahtunut vakava k‰sittelyvirhe, keskeytet‰‰n prosessi")."</p>";
				die();
			}
		}
					...
					if ($solu == '!' or $solu = (float) $solu or $summabudjetti == "on") {
						...
						if (mysql_num_rows($result) > 0) {
							...
							if ($budjrow['summa'] != $solu) {
								...
								// $tall_maara on kantaan tallennettava kappalem‰‰r‰ tuotteita
								$tall_index = 1.00;
								$tall_maara = 0;
								...
								// Jos teet t‰h‰n muutoksia niin tee ne myˆs insert-puolelle.
								...
								elseif ($budj_kohtelu == "isoi" and $solu > 0.00) {
									...
									echo "<p class='error'>".t("Virhe: Tˆrm‰ttiin virheeseen ja emme tallentaneet tietoa %s-tauluun",$kukarow["kieli"],$budj_taulu)."</p>";
									break 3;
								}
							...
							elseif ($tall_maara == 0 and $tall_index == 1 and $solu == 0.00 and $toim == "TUOTE" and $budj_kohtelu == "maara") {
								echo "<p class='error'>".t("Virhe: Ei saa syˆtt‰‰ tyhj‰‰ %s-tauluun", $kukarow["kieli"],$budj_taulu)."</p>";
								break 3;
							}
							elseif ($toim == "TUOTE" and $budj_kohtelu == "isoi") {
								echo "<p class='error'>".t("Virhe: tuotteella %s ei lˆytynyt myynti‰",$kukarow["kieli"],$liitostunnus)." ".substr($kausi,4,2)."/".$edvuosi."</p>";
							}
							else {
								echo "<p class='error'>".t("Virhe: Tˆrm‰ttiin virheeseen ja emme tallentaneet tietoa %s-tauluun",$kukarow["kieli"],$budj_taulu)."</p>";
								break 3;
	...
	if ($toim == "TUOTE") {
		...
		$scheck = "";
		if (isset($summabudjetti) and $summabudjetti) $scheck = 'CHECKED';
	...
	if ($liitostunnukset != "") {
		...
		if ($toim == "TUOTE") {
			...
			$tcheck = "";
			if (isset($tuotteetilmanbudjettia) and $tuotteetilmanbudjettia) $tcheck = 'CHECKED';
	...
	if (isset($mul_osasto) and isset($mul_try) and count($mul_osasto) > 1 and count($mul_try) >= 1) {
		echo "<p class='error'>".t("VIRHE: Et voi valita useita osastoja ja tuoteryhmi‰ kerrallaan")."</p>";
		die();
	}
	...
	if ($submit_button != "" and ($tuoteno != "" or $asiakasid > 0 or $toimittajaid > 0 or $lisa != "" or $lisa_parametri != "" or ($toim == "TUOTE" and $lisa_dynaaminen["tuote"] != "") or ($toim == "ASIAKAS" and $lisa_dynaaminen["asiakas"]) or $liitostunnukset != "") and is_array($tilikaudetrow)) {
		...
		if ($budj_kohtelu == "isoi" and $budj_taso == "summataso") {
			echo "<p class='error'>".t("VIRHE: Ei voida valita indeksi‰ sek‰ jakoa kuukausittain!! Valitse Budjettiluvusta toinen vaihtoehto")."</p>";
			exit;
		}
		...
		if ($summabudjetti == "on") {
			...
			// Mik‰li luot lis‰‰ taikka muokkaat "haen" ja "passaan" muuttujia, niin korjaa myˆs riville 1170 alken piirtoihin.
		...
		while ($raja < substr($tilikaudetrow['tilikausi_loppu'], 0, 7)) {
			...
			if (($budj_taso == "samatasokk" or $budj_taso == "summataso") and $sarakkeet == 1) {
				// n‰ytet‰‰n vain 1 sarake jossa on valitun/syˆtetyn aikakauden
			...
			if ($sarakkeet > 24) {
				echo "<p class='error'>".t("VIRHE: Ei voi tehd‰ yli 2 vuoden budjettia")." !!!<br></p>";
				die();
			}
		}
		...
		function piirra_budj_rivi ($row, $tryrow = "", $ohitus = "", $org_sar = "") {
			...
			for ($k = 0; $k < $sarakkeet; $k++) {
				...
				elseif ($ohitus != "") {
					// mik‰li muutat "poikkeus" tai "poikkeus_haku" nimityksi‰ tai arvoja tai lis‰‰t haaroja, niin muuta/lis‰‰ ne myˆs kohtaan riville 258
					// jossa k‰sitell‰‰n ne arrayn luonnissa
					// ƒLƒ MUUTA "<input type='text' class = '{$row[$budj_sarak]}'"
					// Mik‰li teet muutoksia niin muuta myˆs jqueryyn riville noin 17
		...
		elseif ($tuotteetilmanbudjettia) {
			// Mik‰li ollaan valittu k‰yttˆliittym‰st‰ "N‰yt‰ Vain Ne Tuotteet Joilla Ei Ole Budjettia"
			...
			// Mik‰li lˆytyy yksikin kauden p‰tk‰ miss‰ ei ole arvoa tuotteelle, niin piirret‰‰n

<?php
	...
	if ($tee == 'aja') {
		...
		if (count($tunnukset) > 0) {
							...
							$('#tilaus').change(function(){
								if (this.value != '') {
									$.get('$_SERVER[SCRIPT_NAME]', { tee: 'hae_kommentti', tun: this.value, no_head: 'yes', ohje: 'off' }, function(data){
										$('#message:font').removeClass('error').addClass('ok').text('Lis‰‰ kommentti');
										$('#kommentti').val(data);
									});
								}
								else {
									$('#kommentti').val('');
									$('#message:font').addClass('ok').text('Valitse tilausnumero ja syˆt‰ kommentti');
								}
							});
			...
			echo "<tr><td colspan='3'><div id='message'><font class='ok'>",t("Valitse tilausnumero ja syˆt‰ kommentti"),"</font></div></td></tr>";
			echo "<tr><td>";
			echo t("Lis‰‰ kommentti"),":</td>";

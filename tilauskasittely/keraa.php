    ...
    for ($a=0; $a < count($kerivi); $a++) {
      ...
      if ((trim($varastorekla[$kerivi[$a]]) == trim($vertaus_hylly[$kerivi[$a]])) and $reklahyllyalue[$kerivi[$a]] != '' and $reklahyllynro[$kerivi[$a]] != '') {
        if (kuuluukovarastoon($reklahyllyalue[$kerivi[$a]], $reklahyllynro[$kerivi[$a]], '') == 0) {
          echo "<font class='error'>".t("VIRHE: Tuotenumerolle")." ".$rivin_tuoteno[$kerivi[$a]]." ".t("annettu paikka")." ".$reklahyllyalue[$kerivi[$a]]."-".$reklahyllynro[$kerivi[$a]]."-".$reklahyllyvali[$kerivi[$a]]."-".$reklahyllytaso[$kerivi[$a]]." ".t("ei kuulu mihinkään varastoon")."!</font><br>";
          $virherivi++;
        }
      }
          ...
          if (trim($maara[$apui]) != '') {
            ...
            elseif ($tilrivirow["var"] == 'J' and $maara[$apui] > 0) {
              ...
              if ($maara[$apui] < $jtsek) {
                ...
                $kerattyaik = "''";
                $rkomm      = $tilrivirow["kommentti"];
                if ($yhtiorow["kerasyerat"] == "" and $tilrivirow["perheid"] != 0) {
                  $rperheid  = 0;
                }
                else {
                  $rperheid  = $tilrivirow['perheid'];
            ...
            elseif ((!isset($poikkeama_kasittely[$apui]) or $poikkeama_kasittely[$apui] == "") and $maara[$apui] >= 0 and $maara[$apui] < $tilrivirow['varattu'] and ($otsikkorivi['clearing'] == 'ENNAKKOTILAUS' or $otsikkorivi['clearing'] == 'JT-TILAUS')) {
              ...
              if ($otsikkorivi['clearing'] == 'JT-TILAUS' and $yhtiorow["kerayserat"] == '' and $tilrivirow["perheid"] == 0) {
                $query1 = "SELECT tunnus FROM tilausrivi 
                            WHERE tilausrivi.yhtio    = '$tilrivirow[yhtio]'
                            AND tilausrivi.tunnus     = '$tilrivirow[tunnus]'";
                $prresult = pupe_query($query1);

                if (mysql_num_rows($prresult) == 1) {  
                  $perheup  = "UPDATE tilausrivi SET perheid = 0 yhtio = '$tilrivirow[yhtio]' AND '$tilrivirow[tunnus]'";
                  $perheup  = pupe_query($perheup);
                }

...
if ($_SERVER['REMOTE_ADDR'] !== '127.0.0.1' or $_SERVER['REMOTE_ADDR'] !== '::1') {
	...
	if ($_GET["tee"] == "MASTER_TO_SLAVE_CONNECTION") {
		...
		if (isset($slavedb[1]) and $slavedb[1] != "" and isset($slaveuser[1]) and $slaveuser[1] != "" and isset($slavepass[1]) and $slavepass[1] != "") {
			$link = mysql_connect($slavedb[1], $slaveuser[1], $slavepass[1]) or die ("CRITICAL - Master cannot connect to slave##$STATE_CRITICAL");
			mysql_select_db($dbkanta) or die ("CRITICAL - Pupesoft-database not found on slave##$STATE_CRITICAL");

			if (is_resource($link)) {
				echo "OK - Master connection to slave OK##$STATE_OK";
				exit;
			}
		}
		else {
